...
def look_at_timing_differences(
    data: pl.DataFrame,
    col1_name: str,
    col2_name: str,
    ax: plt.Axes = None,
    ...
    plot_data = data.with_columns(
        ...
        .then(pl.lit("#018f10"))
        .when(pl.col("outcome") == 0)
        .then(pl.lit("#b4b8b4"))
        .otherwise(pl.lit("#f22824"))
        .alias("dot_color")
    )
    ...
    ax.plot(
        x,
        res.intercept + res.slope * time_on_x,
        "k",
        label=f"R2={res.rvalue**2:.3f}\ny0={res.intercept:.2f}ms\nm={res.slope:.2f}",
    )
...
def plot_all_timings(data: pl.DataFrame, only_hit: bool = False) -> None:
    ...
    for i, c in enumerate(timing_cols):
        ...
        ax = look_at_timing_differences(
            data=data, col1_name=c, col2_name="response_latency", ax=ax, only_hit=only_hit
        )
