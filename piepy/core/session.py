...
class SessionMeta:
    ...
    def set_attrs_from_sessiondir(self, sessiondir: str) -> None:
        ...
        if self.imaging_mode == "cam":
            ...
            raise ValueError(
                f"Parsed {self.imaging_mode} as imaging mode, this is not possible, check the session_name!!"
            )
        # userid
        self.user = tmp[-1]
...
class Session:
    ...
    def data(self):
        if len(self.runs) == 1:
            ...
            raise ValueError(
                f"Session has {len(self.runs)} runs, can't get a single session data :("
            )
    ...
    def stats(self):
        if len(self.runs) == 1:
            ...
            raise ValueError(
                f"Session has {len(self.runs)} runs, can't get a single session stats :("
            )
