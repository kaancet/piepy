...
def show_regions(img: np.ndarray, regions: dict, ax=None):
    """Overlay region polygons + names on a frame for visual confirmation.

    img: 2D frame, or a movie ((N,H,W)/(N,1,H,W)) which is mean-projected.
    regions: {name: (V,2) (x,y) verts}. Static; works with any backend
    (inline included). Returns (fig, ax).
    """
    import matplotlib.pyplot as plt

    if np.ndim(img) > 2:
        img = reference_frame(img)
    if ax is None:
        _, ax = plt.subplots()
    ax.imshow(img, cmap="gray")
    for name, verts in regions.items():
        verts = np.asarray(verts, dtype=float)
        ax.fill(verts[:, 0], verts[:, 1], alpha=0.25, edgecolor="r")
        ax.text(
            verts[:, 0].mean(),
            verts[:, 1].mean(),
            name,
