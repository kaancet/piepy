...
class OnePAnalysis(CamDataAnalysis):
        ...
        **kwargs,
        ...
        if dFoverF:
            trial_avg[:, 0, :, :] = trial_avg[:, 0, :, :] / (
                np.mean(trial_avg[: pre + 1, 0, :, :], axis=0) + 3
            )
    ...
    def make_frame_matrix(self) -> None:
        ...
        self.frame_mat = np.zeros(
            (len(stimmed_frames), len(stimmed_frames.columns) + 3), dtype=int
        )
    ...
    def _get_average(
        ...
        mean_mat = np.zeros(
            (
                frame_count,  # min frame count per repeat/trial
                1,  # channel(gray)
                int(self.tif_stack.shape[2] / downsample),  # height
                int(self.tif_stack.shape[3] / downsample),
            )  # width
        )
        prev_last_frame = 0
        for curr_batch in batches:
            ...
            temp_mat = np.zeros(
                (
                    len(curr_batch),
                    1,
                    int(self.tif_stack.shape[2] / downsample),
                    int(self.tif_stack.shape[3] / downsample),
                )
            )
