...
def downsample_movie(movie, block_size=2, func=np.mean):
    ...
    for i in range(movie.shape[0]):
        new_movie[i, :, :] = downsample_image(
            movie[i, :, :], block_size=block_size, func=func
        )
...
def make_square(img):
    ...
    if im_width == im_height:
        ...
        crop_points = [
            ...
            int(np.floor(shape_diff / 2 + im_height)),
        ]
        ...
        crop_points = [
            int(np.floor(shape_diff / 2)),
            int(np.floor(shape_diff / 2 + im_width)),
        ]
        ...
        with tqdm(total=len(os.listdir(experiment_dir))) as pbar:
            for exp in os.listdir(experiment_dir):
                ...
                if ".DS" in exp:
                    ...
                    if downsample:
                        avg_mov_dict[key] = downsample_movie(
                            avg_mov_dict[key],
...
def plot_mag_phase_maps(phase_dict, mag_dict, clim_percentages=[30, 60]):
    ...
    cr = [
        int(phase_dict["M2T"].shape[0] * clim_percentages[0] / 100),
        int(phase_dict["M2T"].shape[1] * clim_percentages[1] / 100),
    ]
    ...
    for i, k in enumerate(phase_dict.keys()):
        ...
        ax_p.set_clim(
            np.percentile(p_img[cr[0] : cr[1], cr[0] : cr[1]], 5),
            np.percentile(p_img[cr[0] : cr[1], cr[0] : cr[1]], 95),
        )
        ...
        ax_m.set_clim(
            np.percentile(m_img[cr[0] : cr[1], cr[0] : cr[1]], 5),
            np.percentile(m_img[cr[0] : cr[1], cr[0] : cr[1]], 95),
        )
