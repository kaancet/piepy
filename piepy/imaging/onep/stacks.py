...
class GenericStack:
    ...
    def export_binary(
        ...
        fname = pjoin(
            "{0}".format(foldername),
            "{4}_{0}_{1}_{2}_{3}.bin".format(*shape[1:], self.dtype, basename),
        )
        if not os.path.isdir(os.path.dirname(fname)):
            os.makedirs(os.path.dirname(fname))
        out = np.memmap(fname, dtype=self.dtype, mode="w+", shape=tuple(shape))
        for c in tqdm(chunks, desc="Exporting binary"):
            if channel is None:
                out[c[0] - start_frame : c[1] - start_frame] = self[c[0] : c[1]]
            else:
                out[c[0] - start_frame : c[1] - start_frame, 0] = self[
                    c[0] : c[1], channel
                ]
        out.flush()
        del out
...
class TiffStack(GenericStack):
    ...
    def _load_substack(self, fileidx, channel=None):
        self.current_stack = self._imread(self.filenames[fileidx]).reshape(
            [-1, *self.dims]
        )
