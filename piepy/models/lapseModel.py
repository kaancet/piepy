...
class LapseModel:
    ...
    def calculate_pr_right(self, input: np.ndarray) -> float:
        ...
        if self.num_lapse_params == 2:
            ...
            prob_right = self.lapse_params[0] + (1 - 2 * self.lapse_params[0]) * softmax
    ...
    def calculate_logits(self, input: np.ndarray) -> np.ndarray:
        ...
        assert (max(prob_right) <= 1) or (min(prob_right) >= 0), (
    ...
    def log_likelihoods(self, data, input, mask, tag) -> np.ndarray:
        ...
        return stats.categorical_logpdf(
            data[:, None, :],
            time_dependent_logits[:, None, None, :],
            mask=mask[:, None, :],
        )
    ...
    def sample(self, input, tag=None, with_noise=True):
        ...
        pr_right, softmax = self.calculate_pr_right(input)  # vectors of length T
        pr_lapse = self.calculate_pr_lapse()
        for t in range(T):
            z_sample[t] = npr.choice(2, p=[(1 - pr_lapse), pr_lapse])
            if z_sample[t] == 0:
                data_sample[t] = npr.choice(2, p=[(1 - softmax[t]), softmax[t]])
                ...
                if self.num_lapse_params == 1:
                    ...
                    data_sample[t] = npr.choice(
                        2, p=[(1 - lapse_pr_right), lapse_pr_right]
                    )
    ...
    @ensure_args_are_lists
    def fit(self, datas, inputs, masks, tags, optimizer="s", num_iters=1000, **kwargs):
        ...
        self.params, self.hessian = self.minimize_loss(
            _objective, num_iters=num_iters, **kwargs
        )

    def minimize_loss(
        self, loss, verbose: bool = False, num_iters: int = 1000
    ) -> np.ndarray:
        # Flatten the loss
