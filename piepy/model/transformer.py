    ...
    def transform_columns(self, col_names: list = None) -> None:
        ...
        if col_names is None:
            ...
            if not isinstance(col_names, (list, np.ndarray)):
                raise TypeError(
                    f"col_names argument must be a list, got {type(col_names)} instead"
                )
            columns = []
            for c in col_names:
...
class GLMHMMTransfromer(DataTransformer):
    ...
    def transform_data(
        self, input_data: pd.DataFrame = None, stim_property: str = "contrast"
    ) -> None:
    ...
    def remap_choice_vector(self) -> None:
        ...
        new_choice_vector = [
            choice_mapping[old_choice]
            for old_choice in self.input_data["choice"].to_numpy()
        ]
    ...
    def set_signed_stim_vector(self, stim_property: str = "contrast") -> None:
        ...
        if stim_property not in valid_props:
            raise ValueError(
                f"{stim_property} is not a valid property, expected one of {valid_props}"
            )

        stim_left = np.nan_to_num(self.input_data[stim_property + "_l"].to_numpy(), nan=0)
        stim_right = np.nan_to_num(
            self.input_data[stim_property + "_r"].to_numpy(), nan=0
        )
    ...
    def set_prev_choice_vector(self) -> None:
        ...
        assert (
            len(np.unique(previous_choice)) <= 2
        ), "previous choice should be in {0, 1}; " + str(np.unique(previous_choice))
    ...
    def create_wsls_covariate(self) -> None:
        ...
        """
        previous_reward = np.vstack(
            [
                np.array(self.output_data["rewarded"][0]).reshape(-1, 1),
                self.output_data["rewarded"],
            ]
        )[:-1]
