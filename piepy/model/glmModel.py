...
class Glm:
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        return stats.categorical_logpdf(
            data[:, None, :], time_dependent_logits[:, :, None, :], mask=mask[:, None, :]
        )
    ...
    @ensure_args_are_lists
    def fit_glm(
        self, datas, inputs, masks, tags, num_iters=1000, optimizer="bfgs", **kwargs
    ):
    ...
    def plot_input_vectors(self, labels_for_plot: list = None) -> plt.figure:
        ...
        plt.subplots_adjust(
            left=0.15, bottom=0.27, right=0.95, top=0.95, wspace=0.3, hspace=0.3
        )
