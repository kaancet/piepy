...
class WheelTrialPlotter(WheelBasePlotter):
    ...
    def trialPicture(self, ax=None, *args, **kwargs):
        ...
        for trial_dict in self.trial_data:
            ...
            trial_dict = reset_times(
                trial_dict, cols_to_reset=cols_to_reset, anchor="trialdur"
            )
            ...
            # stimdur
            ax.axvspan(
                trial_dict["stimdur"][0],
                trial_dict["stimdur"][1],
                color="gray",
                alpha=0.1,
                label="Stim Present",
            )
            ...
            # closedloopdur
            ax.axvspan(
                trial_dict["closedloopdur"][0],
                trial_dict["closedloopdur"][1],
                color="gray",
                alpha=0.2,
            ...
            # draw moments
            ax.plot(
                [trial_dict["reaction_t"], trial_dict["reaction_t"]],
                [-100, 100],
                linewidth=2,
                linestyle=":",
                color="red",
            ...
            ax.plot(
                [trial_dict["avg_lick_t_diff"], trial_dict["avg_lick_t_diff"]],
                [-100, 100],
                linewidth=2,
                linestyle=":",
                color="orange",
        ...
        if not barebones:
            ...
            if not notitle:
                ax.set_title(
                    "{0} {1}\nTrial No: {2} (stim on {3})".format(
                        self.metadata["animalid"],
                        self.metadata["date"],
                        trial_dict["trial_no"],
                        side_txt,
                    ),
