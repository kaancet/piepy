...
class WheelSessionPlotter(WheelBasePlotter):
        ...
        if filters is None:
            ...
            if "trial_interval" in filters.keys():
                if isinstance(data, dict):
                    filtered_data = {
                        k: v[
                            (v["trial_no"] >= filters["trial_interval"][0])
                            & (v["trial_no"] <= filters["trial_interval"][1])
                        ]
                        for k, v in data.items()
                    }
                else:
                    filtered_data = data[
                        (data["trial_no"] >= filters["trial_interval"][0])
                        & (data["trial_no"] <= filters["trial_interval"][1])
                    ]
                ...
                if isinstance(data, dict):
                    filtered_data = {
                        k: v[
                            (
                                v["openstart_absolute"]
                                >= filters["trial_interval_mins"][0] * 60000
                            )
                            & (
                                v["openstart_absolute"]
                                <= filters["trial_interval_mins"][1] * 60000
                            )
                        ]
                        for k, v in data.items()
                    }
    ...
    def psychometric(self, ax=None, *args, **kwargs):
        ...
        for i, p_key in enumerate(self.analyzer.pairs.keys()):
            ...
            p_value = self.analyzer.analysis["mantel_haenzsel_Q"][p_key]["results"][
                "p_value"
            ][0][0]
        ...
        if scope_size > 1:
            jit = np.random.choice(
                np.linspace(-scope_size / 2, scope_size / 2, scope_size),
                size=scope_size,
                replace=False,
            )
        else:
        ...
        for i, scope in enumerate(fitted_data.keys()):
            ...
            if "opto" in scope:
                for i, p_key in enumerate(self.analyzer.pairs.keys()):
                    ...
                    print(pair)
                    if pair[1] == scope:
                        p_value = self.analyzer.analysis["mantel_haenzsel_Q"][p_key][
                            "results"
                        ]["p_value"][0][0]
            ...
            ax.errorbar(
                (100 * np.array(fit_side_data["contrast"])) + jit[i],
    ...
    def performance(self, ax=None, *args, **kwargs):
        ...
        for scope in self.session["data"].stim_data.keys():
            ...
            if "opto" in scope:
                perf_line[0].set_path_effects(
                    [
                        path_effects.Stroke(linewidth=8, foreground="b", alpha=1),
                        path_effects.Normal(),
                    ]
                )
        ...
        if not kwargs.get("notitle", False):
            ax.set_title(
                "{0} Session Performance {1}".format(
                    self.session["meta"].animalid,
                    self.session["meta"].nicedate,
                ),
                fontsize=fontsize + 2,
                fontweight="bold",
            )
    ...
    def responseLatency(self, sep_sides=False, ax=None, *args, **kwargs):
        ...
        for scope in self.session["data"].keys():
            ...
            for i, key in enumerate(data_dict.keys()):
                ...
                if "opto" in scope:
                    resp_time[0].set_path_effects(
                        [
                            path_effects.Stroke(linewidth=7, foreground="b", alpha=1),
                            path_effects.Normal(),
                        ]
                    )
        ...
        ax.plot(
            ...
            linewidth=4,
            color="r",
            label="Response Cutoff({0:.2f} s)".format(cutoff),
        )
        # make it pretty
        ax = self.pretty_axes(ax)
        ax.set_yscale("log")
            ...
            for vpos, c in enumerate(np.unique(scope_data["contrast"]), start=1):
                ...
                for side in np.unique(contrast_data["stim_side"]):
                    ...
                    if "opto" in scope:
                        ...
                        ax.scatter(
                            np.sign(side) * (data_x_jittered + scatter_pos),
                            data,
                            s=20,
                            color=contrast_styles[c]["color"],
                            alpha=0.6,
                            label=c,
                            edgecolors="b",
                            linewidth=2,
    ...
    def performancePerStim(self, ax=None, *args, **kwargs):
        ...
        if not kwargs.get("notitle", False):
            ax.set_title(
                "{0} Answer Distribution {1}".format(
                    self.session["meta"]["animalid"], self.session["meta"]["date"]
                ),
                fontsize=fontsize,
                fontweight="bold",
    ...
    def probability(self, ax=None, *args, **kwargs):
        ...
        if not kwargs.get("barebones", False):
            ...
            if not kwargs.get("notitle", False):
                ax.set_title(
                    "{0} Stimulus Side probabilities {1}".format(
                        self.session["meta"]["animalid"], self.session["meta"]["date"]
                    ),
                    fontsize=fontsize,
                    fontweight="bold",
    ...
    def parameterCompare(self, p1, p2, ax=None, *args, **kwargs):
        ...
        # plotting
        ax.scatter(data[p1], data[p2], c="crimson", s=kwargs.get("markersize", 25))
    ...
    def wheelTrajectory_vert(self, seperate_by="contrast", ax=None, *args, **kwargs):
        ...
        for scope in self.session["data"].keys():
            ...
            for i, s in enumerate(sides, 1):
                ...
                for sep in sep_list:
                    ...
                    temp_slice.loc[:, "wheel"] = temp_slice["wheel"].apply(
                        lambda x: x + s
                    )
                    ...
                    if show_individual:
                        for k, trial in enumerate(temp_slice.itertuples()):
                            if trial_interval is not None:
                                if (
                                    trial.trial_no >= trial_interval[0]
                                    and trial.trial_no <= trial_interval[1]
                                ):
                    ...
                    if s_limit is not None:
                        ...
                        if avg is not None and trial_interval is None:
                            ...
                            avg_line = ax.plot(
                                ...
                                **contrast_styles[sep],
                                label=(
                                    "{0} {1}".format(seperate_by, sep) if i == 1 else "_"
                                ),
                                alpha=1,
                                zorder=2,
                            )
                            ...
                            if "opto" in scope:
                                avg_line[0].set_path_effects(
                                    [
                                        path_effects.Stroke(
                                            linewidth=8, foreground="b", alpha=1
                                        ),
                                        path_effects.Normal(),
                                    ]
                                )
        ...
        ax.plot(
            [-50, -50], ax.get_ylim(), "maroon", linestyle="--", linewidth=2, alpha=0.8
        )
    ...
    def metricSummary(self, *args, **kwargs):
        ...
        ax1 = self.metric(
            "path_surplus", data, ax=ax1, notitle=True, savefig=False, *args, **kwargs
        )

        ax2 = self.fig.add_subplot(222)
        ax2 = self.metric(
            "reaction_t", data, ax=ax2, notitle=True, savefig=False, *args, **kwargs
        )

        ax3 = self.fig.add_subplot(223)
        ax3 = self.metric(
            "avg_lick_t_diff", data, ax=ax3, notitle=True, savefig=False, *args, **kwargs
        )

        ax4 = self.fig.add_subplot(224)
        ax4 = self.metric(
            "avg_speed", data, ax=ax4, notitle=True, savefig=False, *args, **kwargs
        )
    ...
    def sessionSummary(self, *args, **kwargs):
        ...
        gs = self.fig.add_gridspec(
            ...
            wspace=kwargs.get("wspace", 0.75),
            hspace=kwargs.get("hspace", 0.7),
        )
        # info text
        ...
        ax_time = self.responseLatency(
            sep_sides=False, ax=ax_time, notitle=True, showlegend=False, *args, **kwargs
        )
        ...
        ax_cumu = self.lickTotal(
            ax=ax_cumu, notitle=True, showlegend=False, *args, **kwargs
        )
        ...
        ax_violin = self.responsePerStim(
            ax=ax_violin, notitle=True, showlegend=False, savefig=False, *args, **kwargs
        )
        ...
        ax_bars = self.performancePerStim(
            ax=ax_bars, notitle=True, savefig=False, *args, **kwargs
        )
