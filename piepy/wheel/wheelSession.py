...
from piepy.core.session import *
from piepy.model.transformer import GLMHMMTransfromer
from piepy.model.glmModel import Glm
            ...
            for i, _ in enumerate(sfreq):
                ...
                for opto_pattern in pattern_ids:
                    ...
                    try:
                        ...
                        stimuli_data = self.get_subset(
                            data_in,
                            {
                                "spatial_freq": sfreq[i],
...
class WheelStats:
    __slots__ = [
        ...
        "answered_trials",
        "all_correct_percent",
        "novel_correct_percent",
        "answered_correct_percent",
        "easy_answered_trials",
        "easy_answered_correct_percent",
        "left_distribution",
        "right_distribution",
    ...
    def init_from_data(self, data_in: pd.DataFrame):
        ...
        # percents
        self.all_correct_percent = round(
            100 * len(data_in[data_in["answer"] == 1]) / len(data_in), 3
        )
        # self.novel_correct_percent = round(100 * len(novel_data[novel_data['answer']==1]) / len(novel_data),3)
        self.answered_correct_percent = round(
            100 * len(answered_data[answered_data["answer"] == 1]) / len(answered_data), 3
        )

        ## performance on easy trials
        easy_trials = data_in[data_in["contrast"].isin([1.0, 0.5])]
        easy_answered_data = easy_trials[
            (easy_trials["correction"] == 0) & (easy_trials["answer"] != 0)
        ]
        self.easy_answered_trials = len(easy_answered_data)
        self.easy_answered_correct_percent = round(
            100
            * len(easy_answered_data[easy_answered_data["answer"] == 1])
            / len(easy_answered_data),
            3,
        )

        # left
        left_data = data_in[data_in["stim_side"] < 0]
        self.left_distribution = [
            len(left_data[left_data["answer"] == 1]),
            len(left_data[left_data["answer"] == -1]),
            len(left_data[left_data["answer"] == 0]),
        ]
        # right
        right_data = data_in[data_in["stim_side"] > 0]
        self.right_distribution = [
            len(right_data[right_data["answer"] == 1]),
            len(right_data[right_data["answer"] == -1]),
            len(right_data[right_data["answer"] == 0]),
        ]
        ...
        self.bias = np.round(
            (n_right_resp - n_left_resp) / (n_right_resp + n_left_resp), 3
        )
...
class WheelSession(Session):
    ...
    def __init__(self, sessiondir, *args, **kwargs):
        ...
        if self.isSaved() and self.load_flag:
            ...
            self.meta.water_given = round(
                float(np.sum([a[1] for a in self.data.data["reward"] if len(a)])), 3
            )
            if self.meta.water_consumed is not None:
                self.meta.water_per_reward = self.meta.water_consumed / len(
                    self.data.data[self.data.data["answer"] == 1]
                )
            else:
    ...
    def set_meta(self) -> None:
        ...
        self.meta.contrastVector = [
            float(i) for i in self.meta.contrastVector.strip("] [").strip(" ").split(",")
        ]
        ...
        if hasattr(self.meta, "stimRegion"):
            self.meta.stimRegion = [
                float(i) for i in self.meta.stimRegion.strip("] [").strip(" ").split(",")
            ]
    ...
    def get_session_data(self) -> pd.DataFrame:
        ...
        self.states["transition"] = self.states.apply(
            lambda x: self.translate_transition(
                x[self.column_keys["oldState"]], x[self.column_keys["newState"]]
            ),
            axis=1,
        )
        ...
        if session_data.empty:
            print(
                """WARNING THIS SESSION HAS NO DATA
            Possible causes:
    ...
    @staticmethod
    def filter_trials_by_response(
        data_in: WheelData, cutoff_time: float = 1000
    ) -> WheelData:
...
def get_running_stats(data_in: pd.DataFrame, window_size: int = 10) -> pd.DataFrame:
    ...
    # response latency
    data_in.loc[:, "running_response_latency"] = (
        data_in.loc[:, "response_latency"].rolling(window_size, min_periods=5).median()
    )

    # answers
    answers = {"correct": 1, "nogo": 0, "incorrect": -1}
