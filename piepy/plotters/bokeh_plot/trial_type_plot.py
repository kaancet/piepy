...
class TrialTypeBarGraph(Graph):
    ...
    def morph_data(data_in: pl.DataFrame) -> dict:
        ...
        df = df.with_columns(
            pl.concat_list(pl.col(["stim_label", "signed_contrast"])).alias("xaxis")
        )
    ...
    def plot(self) -> None:
        ...
        f = figure(
            title="",
            x_range=FactorRange(*self.cds.data["xaxis"]),
            width=600,
            height=400,
            x_axis_label="Trial Type)",
            y_axis_label="Count",
        )
        ...
        hover = HoverTool(
            tooltips=[("Correct Count", "@correct"), ("Miss Count", "@miss")]
        )
        f.add_tools(hover)
