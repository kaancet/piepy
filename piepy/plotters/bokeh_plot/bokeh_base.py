...
def select_stim_data(data_in: pl.DataFrame, stimkey: str = None, drop_early: bool = True):
    ...
    uniq_keys = (
        data.select(pl.col("stimkey")).drop_nulls().unique().to_series().to_numpy()
    )
    ...
    if stimkey is not None:
        ...
        if len(uniq_keys) == 1:
            ...
            raise ValueError(
                "There is no stimkey in the data, this should not be the case. Check your data!"
            )
...
def set_theme(theme: str = "light") -> None:
    themes_dir = (
        "C:\\Users\\kaan\\code\\visual-perception\\piepy\\plotters\\bokeh_plot\\themes"
    )
    themes_list = os.listdir(themes_dir)
    if not f"{theme}_theme.yml" in themes_list:
        display(
            f"{theme} not in available custom themes. Stting the theme to light for now. Use one of: {themes_list}"
        )
        theme = "light"
