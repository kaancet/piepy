...
class LifeBasePlotter:
    ...
    def __init__(self, animalid: str, dateinterval=None) -> None:
        ...
        self.appended_list = (
            []
        )  # to prevent appending same kind of data twice, holds the appended task types
    ...
    def add_difference_columns(self) -> None:
        ...
        try:
            start_day = self.plot_data[
                self.plot_data["paradigm"].str.contains("training")
            ]["dt_date"].iloc[0]
            sesh_idx = self.plot_data.index[
                self.plot_data["dt_date"] == start_day
            ].to_list()[0]
        ...
        # day diff
        self.plot_data["day_difference"] = dates_to_deltadays(
            self.plot_data["dt_date"].to_numpy(), start_day
        )

        # session_diff
        self.plot_data["session_difference"] = self.plot_data.apply(
            lambda x: (
    ...
    def get_data(self, task_name: str, data_type: str = "summary") -> None:
        ...
        if self.plot_data is None:
            ...
            self.plot_data = self.plot_data.drop_duplicates(
                subset=["date"], ignore_index=True
            )
    ...
    def get_checkpoint_days(self, task_name: str):
        ...
        self.plot_data["rolling_nogo"] = (
            self.plot_data["nogo_percent"].rolling(window=5).mean()
        )
        temp_data = self.plot_data[self.plot_data["paradigm"].str.contains("training")]
        wheel_learn_session_diff = temp_data[temp_data["rolling_nogo"] <= nogo_threshold][
            "session_difference"
        ].iloc[0]
...
class TrainingPlotter(LifeBasePlotter):
    ...
    def add_normalized_weights(self) -> None:
        ...
        # normalized weight
        restriction_start_weight = self.plot_data[
            self.plot_data["paradigm"] == "water restriction start"
        ]["weight"].values
        ...
        self.plot_data["normalized_weight"] = self.plot_data["weight"].apply(
            lambda x: float(x) / start_weight
        )
    ...
    def plot(
        self, plot_range: int = None, diff_type: str = "sessions", **kwargs
    ) -> plt.Axes:
        ...
        # weight
        ax1.plot(
            self.plot_data[self.x_column_name],
            self.plot_data["normalized_weight"],
            color="k",
            linewidth=4,
            marker="o",
        )
        ...
        ax2.plot(
            self.plot_data[self.x_column_name],
            self.plot_data["median_response_time"].to_numpy() / 1000,
            color="#0076d6",
            marker="o",
            markersize=5,
            linewidth=4,
        ...
        ax2.tick_params(
            axis="x", labelsize=15, length=5, width=2, which="major", color="k"
        )
...
class LifeLinePlotter(LifeBasePlotter):
    ...
    def add_level_colors(self, task_name: str) -> None:
        ...
        training_slice = self.plot_data[
            self.plot_data["leveled_paradigm"].str.contains("training")
        ]
        ...
        level_colors = {
            level: current_color_range[i] for i, level in enumerate(uniq_levels)
        }
    ...
    def add_levels(self, task_name: str) -> None:
        ...
        self.plot_data["leveled_paradigm"] = self.plot_data[["paradigm", "level"]].apply(
            lambda x: concat_level(x.paradigm, x.level), axis=1
        )
