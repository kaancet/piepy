...
class WheelResponseTypeBarPlotter(ResponseTypeBarPlotter):
    ...
    def plot(self, ax: plt.Axes = None, padding=0.8, **kwargs) -> plt.Axes:
        ...
        for answer in np.unique(self.plot_data[self.stimkey]["answer"]):
            answer_data = self.plot_data[self.stimkey][
                self.plot_data[self.stimkey]["answer"] == answer
            ]
            counts = [
...
class WheelLickScatterPlotter(LickScatterPlotter):
    ...
    def modify_data(self, *args, **kwargs):
        ...
        if isinstance(self.plot_data, dict):
            for k, v in self.plot_data.items():
                v["response_latency_absolute"] = (
                    v["response_latency"] + v["openstart_absolute"]
                )
        else:
            self.plot_data["response_latency_absolute"] = (
                self.plot_data["response_latency"] + self.plot_data["openstart_absolute"]
            )
    ...
    def plot(
        ...
        **kwargs,
        ...
        for row in self.plot_data[self.stimkey][
            self.plot_data[self.stimkey]["answer"] == 1
        ].itertuples():
            if len(row.reward):
                if len(row.lick):
                    if wrt == "reward":
                        ...
                        wrt_color = "r"
                        ax.scatter(
                            response_time, row.trial_no, c="k", marker="|", s=20, zorder=2
                        )
                    elif wrt == "response":
                        ...
                        ax.scatter(
                            reward, row.trial_no, c="r", marker="|", s=20, zorder=2
                        )
...
class WheelWheelTrajectoryPlotter(WheelTrajectoryPlotter):
    ...
    def __init__(
        self, data: dict, stimkey: str = None, seperate_by: str = "contrast", **kwargs
    ):
        super().__init__(data, stimkey, **kwargs)
    ...
    def seperate_wheel_data(self, seperate_by: str = "contrast"):
        ...
        if seperate_by not in self.plot_data[self.stimkey].columns:
            raise ValueError(
                f"{seperate_by} is not a valid field for this data. try: {self.plot_data.columns}"
            )
        ...
        for i, side in enumerate(sides, start=1):
            side_slice = self.plot_data[self.stimkey][
                self.plot_data[self.stimkey]["stim_side"] == side
            ]
    ...
    def plot(
        ...
        **kwargs,
        ...
        if orientation == "vertical":
            # trigger zones
            ax.plot(
                [0, 0], ax.get_ylim(), "green", linestyle="--", linewidth=2, alpha=0.8
            )
            ax.plot(
                [-50, -50],
                ax.get_ylim(),
            ...
            # trigger zones
            ax.plot(
                ax.get_xlim(), [0, 0], "green", linestyle="--", linewidth=2, alpha=0.8
            )
            ax.plot(
                ax.get_xlim(),
                [-50, -50],
            ...
            ax.plot(
                ax.get_xlim(), [50, 50], "maroon", linestyle="--", linewidth=2, alpha=0.8
            )
...
class WheelSummaryPlotter:
    ...
    def __init__(
        self, data: dict, stimkey: str = None, cutoff_time: int = None, **kwargs
    ) -> None:
    ...
    def init_plotters(self):
        ...
        self.plotters = {
            "performance": WheelPerformancePlotter(cutoff_data, self.stimkey),
            "responsepertype": WheelResponseTimeScatterCloudPlotter(
                cutoff_data, self.stimkey
            ),
    ...
    def plot(self, **kwargs):
        ...
        gs = self.fig.add_gridspec(
            ...
            left=kwargs.get("left", 0.04),
            right=kwargs.get("right", 0.96),
            wspace=kwargs.get("wspace", 0.35),
