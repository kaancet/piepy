...
from ....psychophysics.wheel.detection.wheelDetectionExperimentHub import generate_unique_session_id
...
def _plot_single_panel(ax:plt.Axes,
    ...
    avg_df = (panel_data
                ...
                .agg(
                    [
                        ...
                        pl.col(plot_of).map_elements(mean_confidence_interval,return_dtype=pl.List(pl.Float64)).alias(f"conf_{plot_of}"),
                        pl.concat_list([pl.col(plot_of).median() - pl.col(plot_of).quantile(0.25),
                                        pl.col(plot_of).quantile(0.75) - pl.col(plot_of).median()]).alias(f"iqr_{plot_of}"),
                    ]
        )
        .sort(["opto_pattern"])
    )
    ...
    # shaded CI
    bars = np.array(avg_df[f"{plot_with}_{plot_of}"].to_list()).transpose()
    ax.errorbar(
        avg_df["opto_pattern"].to_numpy(),
        _plotting,
        bars,
...
def plot_hit_rate_change(
    ...
    p_stats:Literal["wilcoxon","paired_t"]="wilcoxon",
    ...
    **kwargs
    ...
    Args:
        ...
        p_stats (Literal["wilcoxon","paired_t"], optional): Statistical test between opto and non-opto. Defaults to "wilcoxon".
    ...
    aggregator.group_data(group_by = ["animalid", "session_no", "stim_type", "stim_side", "contrast", "opto_pattern"])
    ...
    fig, axes_dict = make_named_axes(data=plot_data,
    ...
    zero_nonopto_df = plot_data.filter((pl.col("contrast")==0) & 
    ...
    for s in zero_opto_df["stim_type"].drop_nulls().unique().sort().to_list():
        ax_key = f"{s}_{0.0}"
        ax = axes_dict[ax_key]
        ax = _plot_single_panel(ax,zero_opto_df,
                                plot_of="hit_rate",
                                plot_with=plot_with,
                                mpl_kwargs=mpl_kwargs,
                                **kwargs)
    ...
    for filt_tup in make_subsets(nonzero_data,["stim_type", "contrast"]):
        ...
        ax = axes_dict[ax_key]
        ax = _plot_single_panel(ax,filt_df,
                                plot_of="hit_rate",
                                plot_with=plot_with,
                                mpl_kwargs=mpl_kwargs,
                                **kwargs)
        ...
        if len(uniq_opto_pattern) > 1:
            for i,j in list(itertools.combinations([x for x in range(len(uniq_opto_pattern))], 2)):
                ...
                try:
                    if p_stats == "wilcoxon":
                        res = wilcoxon(hr_1,hr_2)
                    elif p_stats == "paired_t":
                        res = ttest_rel(hr_1,hr_2)
                    p = res.pvalue
                    print(filt_stimtype,filt_contrast,res,flush=True)
                    ax = pval_plotter(ax,p,
                                    pos=[uniq_opto_pattern[i],uniq_opto_pattern[j]],
                                    loc=100)
                except Exception:
                    pass
            ax.tick_params(
    ...
    plt.subplots_adjust(hspace=mpl_kwargs.pop("hspace",0.5),
                        wspace=mpl_kwargs.pop("wspace",0.25))
    return fig
    ...
    for filt_tup in make_subsets(nonzero_data,["stim_type", "contrast"]):
        ...
        if len(uniq_opto_pattern) > 1:
            for i,j in list(itertools.combinations([x for x in range(len(uniq_opto_pattern))], 2)):
                ...
                if p_stats == "wilcoxon":
                    res = wilcoxon(rt_1,rt_2)
                elif p_stats == "paired_t":
                    res = ttest_rel(rt_1,rt_2)
                p = res.pvalue
