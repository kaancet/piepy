...
def plot_delta_effect_contrast(
    ...
    **kwargs,
    ...
    _lin_axis = [
        ...
        for c in plot_data["contrast"].to_list()
    ]
        ...
        for filt_tup in make_subsets(stim_df, ["session_id"]):
            ...
            if not filt_df.is_empty():
                q = (
                    ...
                    .agg([pl.col(effect_on).get(0), pl.col("hit_count").get(0)])
                    .sort(["opto_pattern", "contrast"])
                )

                vals_nonopto = q.filter(pl.col("opto_pattern") == -1)[
                    effect_on
                ].to_numpy()
                contrast_nonopto = q.filter(pl.col("opto_pattern") == -1)[
                    "contrast"
                ].to_list()
                ...
                for o in opto_patterns:
                    vals_opto = q.filter(pl.col("opto_pattern") == o)[
                        effect_on
                    ].to_numpy()
                    contrast_opto = q.filter(pl.col("opto_pattern") == 0)[
                        "contrast"
                    ].to_list()
                    hit_count_opto = q.filter(pl.col("opto_pattern") == o)[
                        "hit_count"
                    ].to_numpy()

                    paired_idx = [
                        i for i, c in enumerate(contrast_nonopto) if c in contrast_opto
                    ]
                    ...
                    if "time" in effect_on:
                        temp[hit_count_opto <= min_trial_count] = np.nan
                        # vals_diff = [
        ...
        for k, v in diff_values.items():
            ...
            for j in range(data_mat.shape[1]):
                if not np.all(np.isnan(data_mat[:, j])):
                    cloud_pos = x_pos_values[0][j] + pos_ofs * kwargs.get(
                        "cloud_offset", 0.25
                    )
                    ...
                    ax = pval_plotter(
                        ...
                        [cloud_pos, cloud_pos],
                        loc=polarity * 100,
                        color=clr.stim_keys[f"{stim_df[0, 'stim_type']}_{int(o)}"][
                            "color"
                        ],
                    )
    ...
    **kwargs,
    ...
    for filt_tup in make_subsets(plot_data, ["session_id"]):
        ...
        if not filt_df.is_empty():
            ...
            for o in opto_patterns:
                ...
                stype_opto = q.filter(pl.col("opto_pattern") == 0)[
                    "stim_type"
                ].to_list()
    ...
    for k, v in diff_values.items():
        ...
        for j in range(data_mat.shape[1]):
            ...
            if effect_on == "hit_rate":
                y_vals = 100 * y_vals
            ax.scatter(x_points, y_vals, zorder=2, c=colors)

            for p1, p2 in list(
                itertools.combinations([x for x in range(data_mat.shape[1])], 2)
            ):
                d1 = data_mat[:, p1]
                d2 = data_mat[:, p2]
        ...
        for i in range(data_mat_x.shape[0]):
            ...
            if effect_on == "hit_rate":
                y_vals = 100 * y_vals
            ax.plot(
                data_mat_x[i, :], y_vals, linewidth=2, alpha=0.2, c="#bfbfbf", zorder=0
            )

    _x_axis_pos = np.mean(np.array(x_pos_values), axis=0, dtype=int)
    ax.set_xticks(_x_axis_pos)
    ax.set_xticklabels([X_AXIS_LABEL_STYPE[i] for i in _x_axis_pos])
...
def plot_delta_effect_areas(
    ...
    **kwargs,
        ...
        if not filt_df.is_empty():
            ...
            for j, a in enumerate(areas):
                ...
                if not _area.is_empty():
                    if (
                        _area.filter((pl.col("opto_pattern") == 0))[0, "hit_count"]
                        >= min_trial_count
                    ):
                        vals_nonopto[j] = _area.filter((pl.col("opto_pattern") == -1))[
                            0, effect_on
                        ]
                        vals_opto[j] = _area.filter(pl.col("opto_pattern") == 0)[
                            0, effect_on
                        ]
    ...
    for c in range(diff_values.shape[1]):
        ...
        ax.scatter(x_points, area_vals, zorder=1, c="#C7C7C7")
    ...
    **kwargs,
    ...
    for filt_tup in make_subsets(plot_data, ["animalid"]):
        ...
        if not filt_df.is_empty():
            ...
            diff_values = np.vstack((diff_values, [diff_noncno, diff_cno]))
            animal_ids.append(a_id)
    ...
    for c in range(diff_values.shape[1]):
        ...
        ax.scatter(x_points, cno_vals, zorder=1, c="#C7C7C7")

    # plot the pvasl
    for p1, p2 in list(
        itertools.combinations([x for x in range(diff_values.shape[1])], 2)
    ):
        d1 = diff_values[:, p1]
        d2 = diff_values[:, p2]
        if p_test == "auto":
