...
def plot_wheel_slope_and_offset_xyerror(
    ...
    **kwargs,
    ...
    for filt_tup in make_subsets(plot_data, seperate_by):
        ...
        _temp_dict[_key] = np.hstack(
            (mov_peak_speed.reshape(-1, 1), mov_onset.reshape(-1, 1))
        )
        ...
        ax._scatter(
            mov_peak_speed, mov_onset, c=clr["color"], alpha=0.2, mpl_kwargs=mpl_kwargs
        )
    ...
    for i, keys in enumerate(list(itertools.combinations(list(_temp_dict.keys()), 2))):
        ...
        ax = pval_plotter(ax, p, pos=[x1, x2], loc=700 + i * 10, tail_height=0.001)
...
def plot_wheel_slope_and_offset_ellipse(
    ...
    **kwargs,
    ...
    for filt_tup in make_subsets(plot_data, seperate_by):
        ...
        x_d = np.linspace(
            np.nanmin(mov_peak_speed) - 1, np.nanmax(mov_peak_speed) + 1, 100
        )
...
def plot_all_wheel_slope_and_offset_ellipse(
    ...
    **kwargs,
    ...
    for filt_tup in make_subsets(data, ["opto_pattern", "stim_type"]):
        ...
        f = plot_wheel_slope_and_offset_ellipse(
            data=filt_df,
