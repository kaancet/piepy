...
class DetectionPsychometricPlotter(BasePlotter):
    ...
    def plot(
        ...
        **kwargs,
        ...
        q = nonearly_data.sort(["stimkey", "opto_pattern", "contrast"], descending=True)
        ...
        for filt_tup in self.subsets(q, ["stimkey", "stim_side"]):
            ...
            if not filt_df.is_empty():
                ...
                if jitter is not None:
                    jittered_offset = np.array(
                        [np.random.uniform(0, jitter) * c for c in contrast]
                    )
                ...
                ax.errorbar(
                    ...
                    color=(
                        self.color.stim_keys[filt_key]["color"]
                        if color is None
                        else color
                    ),
        ...
        if xaxis_type == "log":
            ...
            ax.xaxis.set_minor_locator(
                ticker.LogLocator(base=10.0, subs=np.linspace(0.1, 1, 9, endpoint=False))
            )
            ...
            x_ticks = nonearly_data["linear_contrast_idx"].unique().sort().to_numpy()
            x_labels = nonearly_data["signed_contrast"].unique().sort().to_numpy()
            # x_ticks = np.arange(-(len(temp) - 1) / 2, (len(temp) - 1) / 2 + 1)
            x_t = {x_labels[i]: t for i, t in enumerate(x_ticks)}
            ax.set_xticks(x_ticks)
...
class DetectionResponseHistogramPlotter(ResponseTimeHistogramPlotter):
    ...
    def shuffle_times(x_in, n_shuffle: int = 1000) -> np.ndarray:
        ...
        x_temp = x_in.copy()
        _shuffle1 = np.zeros((n_shuffle, x_in.shape[1]))

        for i in range(n_shuffle):
            gen.shuffle(x_temp, axis=1)
            _shuffle1[i, :] = x_temp.reshape(1, -1)

        idx = np.random.rand(*_shuffle1.shape).argsort(axis=0)
        shuffled_matrix = np.take_along_axis(_shuffle1, idx, axis=0)
    ...
    def make_agg_data(self):
        ...
        q = (
            self.plot_data.lazy()
            .group_by(["stim_type", "contrast", "opto"])
            .agg(
                [
    ...
    def plot(
        ...
        **kwargs,
        ...
        self.plot_data = self.plot_data.with_columns(
            ...
            .then(pl.col(reaction_of))
            .otherwise(-(pl.col("t_blank_dur") - pl.col(reaction_of)))
            .alias("blanked_response_latency")
        )
        ...
        ax = self.__plot__(ax, counts, bins, color="gray", label="Early")
        ...
        for i, t in enumerate(uniq_type):
            for o in uniq_opto:
                ...
                if len(filt_df):
                    ...
                    resp_times = filt_df[reaction_of].to_numpy()
        ...
        # do bootstrap
        # all_resp_times = self.plot_data["blanked_response_latency"].to_numpy()
        # # shuffled = self.shuffle_times(all_resp_times)
        # _bins = np.arange(-4000,1000,bin_width)
        # boot_hists = np.zeros((n_shuffle,len(_bins)-1))
        # for i,_ in enumerate(boot_hists):
        #     _bootstrapped = np.random.choice(all_resp_times,len(all_resp_times),replace=True)
        #     counts,_ = np.histogram(_bootstrapped,_bins)
        #     boot_hists[i,:] = counts.reshape(1,-1)

        # #mean & std
        # boot_mean = np.mean(boot_hists,axis=0)
        # boot_std = np.std(boot_hists,axis=0)

        # ax.fill_between(_bins[1:],boot_mean-boot_std,boot_mean+boot_std,color='dimgrey',alpha=0.4,zorder=2)
        # ax.plot(_bins[1:],boot_mean,color='dimgrey',alpha=0.6,linewidth=2,zorder=3)
        # ax.legend(
        #     loc="center left",
    ...
    def plot(self, ax: plt.Axes = None, bar_width: float = 0.4, **kwargs) -> plt.Axes:
        ...
        for i, k in enumerate(uniq_k, start=2):
            ...
            ax = self.__plot__(
                ...
                width=bar_width,
                color=[
                    self.color.outcome_keys[str(0)]["color"],
...
class DetectionResponseScatterPlotter(BasePlotter):
    ...
    def __plot_density__(ax, x_bins, y_dens, **kwargs):
        ax.plot(
            x_bins[1:], y_dens, c="dimgrey", alpha=0.8, linewidth=3, **kwargs
        )  # right edges
        return ax
    ...
    def set_wrt_response_plot_data(self, wrt="sorted") -> np.ndarray:
        ...
        if wrt == "sorted":
            ...
            d["wrt_response_latency"] = d[
                ["outcome", "blank_time", "response_latency"]
            ].apply(
                lambda x: (
                    x["response_latency"] + x["blank_time"]
                    if x["outcome"] == 1
                    else x["response_latency"]
                ),
                axis=1,
            )
            ...
            d["wrt_response_latency"] = d[
                ["outcome", "blank_time", "response_latency"]
            ].apply(lambda x: x["response_latency"] - x["blank_time"], axis=1)
        else:
    ...
    def plot(
        ...
        **kwargs,
        ...
        times_arr = []
        if blanks == "sorted":
            sorted_data = self.plot_data[self.stimkey].sort_values(
                "blank_time", ascending=False
            )
        ...
        ax.set_yticks(
            [
                i
                for i in range(len(self.plot_data[self.stimkey]))
                if i >= 0 and i % 50 == 0
            ]
        )
...
class DetectionLickScatterPlotter(LickScatterPlotter):
    ...
    def plot(
        ...
        **kwargs,
        ...
        lick_data = self.plot_data.with_columns(
            [
                (
                    pl.when(pl.col("outcome") != -1)
                ...
                (
                    ...
                    + pl.col("open_start_absolute")
                ).alias("response_latency_absolute"),
            ]
        )
...
class DetectionWheelTrajectoryPlotter(WheelTrajectoryPlotter):
    ...
    def plot(
        ...
        **kwargs,
        ...
        ax = super().plot(time_lims=time_lims, traj_lims=traj_lims, **kwargs)
