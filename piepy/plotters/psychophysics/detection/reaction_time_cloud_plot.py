...
def plot_reaction_time_cloud(
    ...
    **kwargs,
    ...
    if combine_sides:
        _grouper = ["stim_type", "contrast", "opto_pattern"]
        _subsetter = ["contrast"]
        _axer = "contrast"
    else:
        _grouper = ["stim_type", "stim_side", "contrast", "opto_pattern"]
        _subsetter = ["stim_side", "contrast"]
    ...
    for filt_tup in make_subsets(grouped_nonearly_data, _subsetter):
        filt_df = filt_tup[-1]
    ...
    x_labels = grouped_nonearly_data[_axer].unique().sort().to_numpy()
