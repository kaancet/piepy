...
class WheelDetectionPerformanceProgressionPlotter(BehaviorBasePlotter):
    def __init__(
        self,
        data: MouseData = None,
        **kwargs,
    ...
    @staticmethod
    def get_fa_rate(arr: np.ndarray) -> np.ndarray:
        """ """
        arr = arr.astype(float)
        # convert non earlies(0,1) to np.nan
        arr[arr != -1] = np.nan
        # ultiply by -1 to convert the early(-1) to 1
        arr = arr * -1
        # do cumsum
        _false_alarms = np.nancumsum(arr)
        _all_trials = np.arange(1, len(_false_alarms) + 1)
        return (_false_alarms / _all_trials) * 100
    ...
    def plot_hit_rate(
        ...
        **kwargs,
        ...
        """Plots the hit rate progression with respect to total trial count"""
        if ax is None:
            self.fig = plt.figure(figsize=kwargs.pop("figsize", (8, 8)))
            ax = self.fig.add_subplot(1, 1, 1)

        return_plotted_data = kwargs.pop("return_data", False)
        session_limit = kwargs.pop("session_limit", None)

        if session_limit is not None:
            _data = self.plot_data.filter(pl.col("session_no") <= session_limit)
            _data_summary = self.summary_data.filter(
                pl.col("session_no") <= session_limit
            )
        else:
            _data = self.plot_data
            _data_summary = self.summary_data

        if self.plot_type == "cumulative":
            if easy_only:
                _data = _data.filter(
                    (pl.col("contrast").is_in([100, 50])) & (pl.col("opto") == False)
                )
                _y = self.get_hit_rate(_data["outcome"].to_numpy())
                _x = np.arange(1, len(_y) + 1)
            else:
                _x = _data["cumul_trial_no"].to_numpy()
                _y = self.get_hit_rate(_data["outcome"].to_numpy())

            if window_width is not None:
                _y = uniform_filter1d(_y, size=window_width)
            ax.set_xlabel("Trials")

        elif self.plot_type == "summary":
            if easy_only:
                _y = _data_summary["easy_hit_rate"].to_numpy()
            else:
                _y = _data_summary["hit_rate"].to_numpy()

            _x = _data_summary["session_no"].to_numpy() - 1
            ax.set_xlabel("Session From 1st")
    ...
    def plot_fa_rate(
        self, ax: plt.Axes = None, window_width: int = None, **kwargs
    ) -> plt.Axes:
        """Plots the false alarm progression with respect to total trial count"""
        if ax is None:
            self.fig = plt.figure(figsize=kwargs.pop("figsize", (8, 8)))
            ax = self.fig.add_subplot(1, 1, 1)
        ...
        if self.plot_type == "cumulative":
            ...
            _x = _data["cumul_trial_no"].to_numpy()
            _y = self.get_fa_rate(_data["outcome"].to_numpy())
            ...
            _x = _data_summary["session_no"].to_numpy() - 1
            _y = _data_summary["false_alarm_rate"].to_numpy()
        ...
        if self.plot_type == "cumulative":
            if easy_only:
                ...
                _data = _data.with_columns(
                    rolling_median=pl.col("response_latency").rolling_median(
                        window_size=window_width
                    )
                )
                _y = _data["rolling_median"].to_numpy()
                _x = np.arange(1, len(_y) + 1)
            else:
                _x = _data["cumul_trial_no"].to_numpy()
                _data = _data.with_columns(
            ...
            if easy_only:
                _y = _data_summary["easy_median_response_latency"].to_numpy()
            else:
                _y = _data_summary["median_response_latency"].to_numpy()
    ...
    def plot_summary(
        self, ax: plt.Axes = None, easy_only: bool = True, window_width: int = None
    ) -> plt.Axes:
        pass
