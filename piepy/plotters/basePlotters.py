...
class BasePlotter:
    ...
    def __init__(self, data: pl.DataFrame, **kwargs):
        ...
        self.color.check_contrast_colors(
            self.plot_data["contrast"].drop_nulls().unique().to_list()
        )
    ...
    def make_linear_contrast_axis(data) -> dict:
        ...
        pos_contrast = (
            data.filter(pl.col("stim_side") == "contra")["contrast"]
            .drop_nulls()
            .unique()
            .sort()
            .to_numpy()
        ...
        neg_contrast = (
            data.filter(pl.col("stim_side") == "ipsi")["signed_contrast"]
            .drop_nulls()
            .unique()
            .sort()
            .to_numpy()
        ...
        pos_part = {
            c: idx
            for c, idx in zip(
                pos_contrast, np.linspace(1, len(pos_contrast), len(pos_contrast))
            )
        }
        neg_part = {
            c: idx
            for c, idx in zip(
                neg_contrast, np.linspace(-len(neg_contrast), -1, len(neg_contrast))
            )
        }
        ...
        for sep in seperate_vals:
            ...
            if running_window is not None:
                data2plot = data2plot.with_columns(
                    pl.col(reaction_of)
                    .rolling_median(running_window)
                    .alias("running_reaction_time")
                )
    ...
    def plot(self, ax: plt.axes = None, plot_in_time: bool = False, **kwargs) -> plt.axes:
        ...
        if len(all_licks):
            if plot_in_time:
                ...
                all_licks[:, 0] = (
                    all_licks[:, 0] / np.max(all_licks[:, 0])
                ) * self.plot_data[self.stimkey]["trial_no"].iloc[-1]
                y_axis_ = np.interp(
                    self.plot_data[self.stimkey]["trial_no"],
                    all_licks[:, 0],
                    all_licks[:, 1],
                )
                x_label_ = "Trial No"
