    ...
    def filter_dates(self, dateinterval: list = None) -> None:
        ...
        if not isinstance(dateinterval, list):
            ...
            if len(dateinterval) != 2:
                raise ValueError(
                    f"dateinterval argument needs to have 2 dates, got {len(dateinterval)}"
                )
        ...
        self.summary_data = self.summary_data[
            (self.summary_data["dt_date"] >= dateinterval_dt[0])
            & (self.summary_data["dt_date"] <= dateinterval_dt[1])
        ]
        self.cumul_data = self.cumul_data[
            (self.cumul_data["dt_date"] >= dateinterval_dt[0])
            & (self.cumul_data["dt_date"] <= dateinterval_dt[1])
        ]
        display(
            f"Filtered data between the dates {dateinterval[0]} - {dateinterval[1]} !"
        )
