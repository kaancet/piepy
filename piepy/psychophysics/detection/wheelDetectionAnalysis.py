...
class DetectionAnalysis:
        ...
        q = (
            ...
            .agg(
                [
                    ...
                    (
                        pl.col("pos_reaction_time")
                        .median()
                        .alias("median_pos_reaction_time")
                    ),
    ...
    def get_deltahits(self) -> pl.DataFrame:
            ...
            .agg(
                [
                    ...
                    (pl.col("hit_rate").first() - pl.col("hit_rate").last()).alias(
                        "delta_HR"
                    ),  # TODO:ASSUMES ONE TARGET FOR NOW
                    (pl.col("confs").first() + pl.col("confs").last()).alias(
                        "delta_HR_err"
                    ),
        ...
        df = df.with_columns(
            (pl.col("delta_HR") / (pl.col("base_HR"))).alias("percent_delta_HR")
        )
        df = df.with_columns(
            (
                pl.col("percent_delta_HR")
                * (
                    (pl.col("delta_HR_err") / pl.col("delta_HR"))
                    + (pl.col("base_HR_err")) / (pl.col("base_HR"))
                )
        ...
        df = df.with_columns(
            (pl.col("delta_HR") / (pl.col("base_HR") - baseline_hr)).alias(
                "percent_delta_HR-baseline"
            )
        )

        #  ΔHR's ratio to baseline
        df = df.with_columns(
            (pl.col("delta_HR") / baseline_hr).alias("delta_HR_baseline")
        )
        ...
        df = df.with_columns(
            [
                (
                    (
                        pl.col("delta_HR") / pl.col("base_HR")
                        - baseline_hr / pl.col("base_HR")
                    )
                    / (
                        pl.col("delta_HR") / pl.col("base_HR")
                        + baseline_hr / pl.col("base_HR")
