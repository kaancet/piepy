...
class WheelGroupedAggregator:
        ...
        if self.data is not None:
            for o in outcomes:
                if o not in self.data["outcome"]:
                    raise ValueError(
                        f"{o} is not a valid outcome type that is present in the outcome column of the data"
                    )

            if len(outcomes) < self.data["outcome"].drop_nulls().n_unique():
                print(
                    f" >WARNING< There are unused outcome values {outcomes} vs {self.data['outcome'].unique().to_list()}"
                )
        ...
        if self.outcomes is not None:
            for o in self.outcomes:
                if o not in data["outcome"]:
                    raise ValueError(
                        "Provided dataframe has not all the values in corresponding to previously set outcomes!"
                    )

            if len(self.outcomes) < data["outcome"].drop_nulls().n_unique():
                print(
                    f" >WARNING< There are unused outcome values {self.outcomes} vs {data['outcome'].unique().to_list()}"
                )
        self.data = data
    ...
    def confidence95(ups: float, downs: float):
        ...
        z = 1.96
        bound_lower = (
            hr + (z**2) / (2 * n) - z * np.sqrt((hr * (1 - hr) + (z**2) / (4 * n)) / n)
        ) / (1 + (z**2) / n)
        bound_upper = (
            hr + (z**2) / (2 * n) + z * np.sqrt((hr * (1 - hr) + (z**2) / (4 * n)) / n)
        ) / (1 + (z**2) / n)
