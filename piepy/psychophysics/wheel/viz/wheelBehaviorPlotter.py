...
class WheelBehaviorPlotter(WheelBasePlotter):
    ...
    def prep_text(self):
        ...
        to_add = {
            ...
            "start_date": dt.strftime(
                self.behavior_data["session_date"].iloc[0], "%d %b %y"
            ),
            "last_date": dt.strftime(
                self.behavior_data["session_date"].iloc[-1], "%d %b %y"
            ),
        }
    ...
    def append_plot_data(self):
        ...
        # set edge colors
        self.behavior_data["edge"] = self.behavior_data["level"].apply(
            lambda x: "blue" if "opto" in x else "white"
        )
        self.behavior_data.loc[self.behavior_data["level"] == "level1", "edge"] = "silver"
        self.behavior_data.loc[self.behavior_data["level"] == "level2", "edge"] = (
            "orangered"
        )
        self.behavior_data.loc[self.behavior_data["level"] == "level3", "edge"] = (
            "magenta"
        )
    ...
    def plot_mondays(self, bd, ax):
        ...
        ylims = ax.get_ylim()
        for row in mondays:
            ax.plot([row, row], ylims, color="gray", linewidth="4", alpha=0.5, zorder=1)
    ...
    def pool_data(self):
        ...
        for row in data.to_dict(orient="records"):
            ...
            for k in data.keys():
                ...
                sesh_data_df["total_trial_no"] = (
                    sesh_data_df["trial_no"] + prev_day_last_trial
                )
                prev_day_last_trial = sesh_data_df["total_trial_no"].iloc[-1]

                if k in self.big_df.keys():
                    self.big_df[k] = pd.concat(
                        [self.big_df[k], sesh_data_df], ignore_index=True, sort=False
                    )
                else:
                    self.big_df[k] = sesh_data_df

        for k in self.big_df.keys():
            self.big_df[k] = get_running_stats(
                self.big_df[k], stats=running_stats, window_size=100, copy_data=False
            )
    ...
    def save(self, plotkey):
        ...
        if self.savefig:
            ...
            savename = "{0}_{1}_{2}.pdf".format(self.baredate, self.animalid, plotkey)
    ...
    def plot(self, plt_func, func_params=[], closefig=False, *args, **kwargs):
        ...
        if plt_func not in callables:
            display(
                "{0} not a plotter function for WheelBehaviorPlotter try {1}".format(
    ...
    def weight(self, ax=None, *args, **kwargs):
        ...
        if level == "all":
            ...
            if level not in levels:
                raise ValueError(
                    "Level {0} does not exist, try one of {1}".format(levels)
                )
            else:
    ...
    def stackedSummary(self, days=None, *args, **kwargs):
        ...
        if days is None:
            ...
            raise ValueError(
                "Days can only be a scalar integer or list, got {0}".format(type(days))
            )
        nrows = 4
        ncols = len(days)
            ...
            for col, day in enumerate(days, 1):
                ...
                if row == 0:
                    ax.set_title(
                        "Day {0}: {1} Trials\n{2}".format(
                            day,
                            day_session.session["summaries"]["overall"]["novel_trials"],
                            day_date,
                        ),
                        fontsize=fontsize,
                    )
    ...
    def performance(self, plt_mode="discrete", ax=None, *args, **kwargs):
        ...
        if plt_mode == "discrete":
            ...
            for k in self.big_df.keys():
                ax.plot(
                    self.big_df[k]["total_trial_no"],
                    100 * self.big_df[k]["fraction_correct"],
                    color="darkgreen",
                ...
                for s in np.unique(self.big_df[k]["session_no"]):
                    temp = self.big_df[k][self.big_df[k]["session_no"] == s][
                        "total_trial_no"
                    ]
                    ax.axvspan(
                        temp.iloc[0],
    ...
    def responseTimes(self, plt_mode="discrete", ax=None, *args, **kwargs):
        ...
        latency_avg = [
            sesh.session["summaries"]["overall"]["latency"] / 1000
            for sesh in bd["session"]
        ]
        ...
        if plt_mode == "discrete":
            ...
            for k in self.big_df.keys():
                ax.plot(
                    self.big_df[k]["total_trial_no"],
                    self.big_df[k]["running_response_latency"] / 1000,
                    color="royalblue",
                    linewidth=kwargs.get("linewidth", 5),
    ...
    def trialDistributions(self, ax=None, *args, **kwargs):
        ...
        left = np.asarray(
            [
                sesh.session["summaries"]["overall"]["left_profile"]
                for sesh in bd["session"]
            ]
        )
        right = np.asarray(
            [
                sesh.session["summaries"]["overall"]["right_profile"]
                for sesh in bd["session"]
            ]
        )
    ...
    def bias(self, ax=None, *args, **kwargs):
        ...
        if not kwargs.get("notitle", False):
            ax.set_title(
                "{0} Bias Progression".format(self.animalid),
                fontsize=fontsize + 2,
                fontweight="bold",
            )
        if kwargs.get("showlegend", True):
    ...
    def psychometric(self, pool_depth=5, ax=None, *args, **kwargs):
            ...
            for k in row["session"].session["novel_stim_data"].keys():
                if k != "overall":
                    if k not in stim_type_dict.keys():
                        ...
                        stim_type_dict[k] = pd.concat(
                            [
                                stim_type_dict[k],
                                row["session"].session["novel_stim_data"][k],
                            ],
                            ignore_index=True,
                            sort=False,
                        )
        ...
        for scope, data in fitted_data.items():
            ...
            ax.errorbar(
                100 * np.array(fit_side_data["contrast"]),
                fit_side_data["percentage"],
                fit_side_data["confidence"],
            ...
            ax.plot(
                100 * fit_side_data["fitted_x"],
                fit_side_data["fitted_y"],
        ...
        if not kwargs.get("notitle", False):
            ax.set_title(
                "{0} Pooled Psychometric Curve".format(self.animalid),
                fontsize=fontsize + 2,
                fontweight="bold",
            )
        if kwargs.get("showlegend", True):
    ...
    def behaviorSummary(self, *args, **kwargs):
        ...
        ax_bias = self.bias(
            ax=ax_bias, notitle=True, savefig=False, showlegend=False, *args, **kwargs
        )

        # psychometric
        ax_psycho = self.fig.add_subplot(gs[2, 0])
        ax_psycho = self.psychometric(
            ax=ax_psycho, notitle=True, savefig=False, *args, **kwargs
        )
        ...
        ax_correct = self.performance(
            ax=ax_correct, notitle=True, savefig=False, showlegend=False, *args, **kwargs
        )
        ...
        ax_response = self.responseTimes(
            ax=ax_response, notitle=True, savefig=False, showlegend=False, *args, **kwargs
        )
        ...
        ax_dist = self.trialDistributions(
            ax=ax_dist, notitle=True, savefig=False, *args, **kwargs
        )
        ...
        ax_water = self.water(
            ax=ax_water, notitle=True, savefig=False, showlegend=False, *args, **kwargs
        )
        ...
        ax_weight = self.weight(
            ax=ax_weight, notitle=True, savefig=False, showlegend=False, *args, **kwargs
        )
