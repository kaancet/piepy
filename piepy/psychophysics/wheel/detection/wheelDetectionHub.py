...
class WheelDetectionHub(TaskHub):
    ...
    def make_summary_data(self, data: pl.DataFrame | None = None) -> pl.DataFrame:
        ...
        df = (
            ...
            .agg(
                ...
                + [
                    pl.col(k).unique(maintain_order=True)
    ...
    def trial_count_more_than(
        self, trial_count: float, data: pl.DataFrame | None = None
    ...
    def ez_hit_rate_more_than(
        self, hit_rate_thresh: float, data: pl.DataFrame | None = None
    ) -> pl.DataFrame:
    ...
    def fa_rate_less_than(
        self, fa_rate_thresh: float, data: pl.DataFrame | None = None
    ) -> pl.DataFrame:
        """Ease of access function to filter by the easy (>=50% contrast) trial hit rate
        ...
        for filt_tup in make_subsets(data, ["session_id"]):
            ...
            if len(hr_df) >= 1:
                ...
                if len(trial_df):
                    ...
                    selected_session = trial_df.sort(
                        ["stat_stim_trial_count", "stat_easy_hit_rate"]
                    )[-1, "session_id"]
                else:
                    # couldn't successfuly filter for trial count, sort and pick the best from hit rate filtered
                    selected_session = hr_df.sort(
                        ["stat_stim_trial_count", "stat_easy_hit_rate"]
                    )[-1, "session_id"]
            else:
                ...
                if not strict_performance:
                    ...
                    if len(trial_df):
                        ...
                        print(
                            f"{a_id}_{opto_target} doesn't have a session for hit rate >={min_ez_hit_rate} and stim_trial_count>={min_stim_trial_count}"
                        )
                else:
                    print(
                        f"{a_id}_{opto_target} doesn't have a session with hit rate >={min_ez_hit_rate} and stim_trial_count>={min_stim_trial_count}, removing..."
                    )
    ...
    def filter_by_areas(
        ...
        **kwargs,
        ...
        _temp = filt_df.group_by(["area", "animalid"]).agg(
            pl.col("*").unique(maintain_order=True)
        )
        with pl.StringCache():
        ...
        single_session_ids = (
            _temp.filter(pl.col("session_id").list.len() == 1)["session_id"]
            .explode()
            .to_list()
        )
        multi_sessions_ids = (
    ...
    def filter_by_animals(
        ...
        **kwargs,
        ...
        _temp = (
            filt_df.group_by(["animalid", "area"])
            .agg(pl.col("*").unique(maintain_order=True))
            .sort("animalid", descending=False)
        )
