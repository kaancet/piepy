...
def find_reaction_point(arr, start_idx=0, end_idx=-1, window_T=3, delta=1):
    ...
    for i in range(len(path_segments) - 1):
        ...
        try:
            ...
            slope = (
                [path_segments[i][0], model.coef_[0]]
                if model.coef_[0] > slope[1]
                else slope
            )
        except:
            print(arr)
        ...
        if col in trial_in.keys():
            if col == "wheel":
                if len(trial_in["wheel"]):
                    ...
                    trial_in["wheel"] = np.hstack(
                        (time_synced, np.array(x[:, 1]).reshape(-1, 1))
                    )
            elif col == "lick":
            ...
            print(
                "Column name {0} not in trial data keys. Skipping time reset".format(col)
            )
...
def get_trajectory_avg(
    wheel_time: np.ndarray,
        ...
        for i, wheel_traj in enumerate(wheel_pos):
            if wheel_traj is not None:
                wheel_interp = np.interp(
                    t, wheel_time[i], wheel_traj, left=np.nan, right=np.nan
                ).reshape(1, -1)
...
def normalized_diff(hit_trials, miss_trials, column):
    ...
    norm_diff = (np.nanmean(miss) - np.nanmean(hit)) / (
        np.nanmean(miss) + np.nanmean(hit)
    )
...
def samples_to_cm(
    positions, wheel_diameter=WHEEL_DIAMETER, resolution=WHEEL_TICKS_PER_REV
):
    """
...
def velocity_filtered(pos, fs, corner_frequency=20, order=8):
    ...
    sos = scipy.signal.butter(
        **{"N": order, "Wn": corner_frequency / fs * 2, "btype": "lowpass"}, output="sos"
    )
