        ...
        if screen_array is None:
            ...
            if len(screen_array) == 2:
                ...
                raise ScreenPulseError(
                    "More than 2 (stim ON - stim oFF) screen pulses for the trial",
                    where=self._trial["trial_no"],
                    hint="Check if riglog has appropriate amount of screen pulses logged nvisual_rials X 2)",
                )
        # else:
        #     raise ScreenPulseError(
