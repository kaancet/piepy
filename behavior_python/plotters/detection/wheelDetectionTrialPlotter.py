...
class DetectionTrialPlotter:
    ...
    def plot(self,ax:plt.Axes=None,trial_no:int=3,t_lim:list=None,**kwargs):
        ...
        # plot the regions
        # quiescence
        ax.axvspan(self.trial_data[0,"t_trialstart_reset"],
                   self.trial_data[0,"t_trialstart_reset"] + self.trial_data[0,"t_quiescence_dur"],
                   color='gray',alpha=0.3,label=f'Trial Prep. ({round(self.trial_data[0,"t_quiescence_dur"],1)}ms)')
        # blank
        ax.axvspan(self.trial_data[0,"t_trialstart_reset"] + self.trial_data[0,"t_quiescence_dur"],
                   self.trial_data[0,"t_stimstart_rig_reset"],
                   color='orange',alpha=0.3,label=f'Wait for Stim ({round(self.trial_data[0,"t_blank_dur"],1)}ms)')
        
        ax.axvspan(self.trial_data[0,"t_stimstart_rig_reset"],
                   self.trial_data[0,"t_stimend_rig_reset"],
                   color='green',alpha=0.3,label='Response Window')
        
        if self.trial_data[0,'opto_pattern']!=-1 and self.trial_data[0,'outcome']!=-1:
