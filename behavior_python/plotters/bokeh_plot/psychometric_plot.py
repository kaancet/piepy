...
class PsychometricGraph(Graph):
    ...
    def set_cds(self,data:pl.DataFrame,**kwargs) -> None:
        ...
        baseline_df = q.filter((pl.col('opto_pattern')==-1) & 
                                (pl.col('stim_side')=='catch'))
        temp_curves['baseline'] = {'y' : [100*np.mean(baseline_df['hit_rate'].to_list())]}
    ...
    def plot(self,**kwargs) -> None:
        f = figure(title="", width=450,height=400,
                   x_axis_label='Contrast(%)', y_axis_label='Hit Rate(%)',
                   x_range=(-3, 3),y_range=(-5, 105))
        ...
        for kk,source_c in self.cds_curves.items():
            if kk == 'baseline':
                f.hspan(y='y',source=source_c,color='#b00000',line_dash='dotted',line_width=3)
                
        for k,source in self.cds_dots.items():
            f.circle(x='contrast',y='hit_rate',
                     color='color',size=15,line_alpha=0,
                     source=source)
            
            f.add_layout(
                Whisker(source=source,base='contrast',upper='confs_up',lower='confs_down',
                        line_color='color',
                        line_width=3,
        ...
        hover = HoverTool(tooltips=[('Contrast', '@signed_contrast'),
