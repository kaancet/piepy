...
class ReactionTimeScatterGraph(Graph):
    ...
    def set_cds(self, data:pl.DataFrame) -> None:
        ...
        colors_curve = [self.color.stim_keys[c]['color'] for c in df_curves['stimkey'].to_list()]
        df_curves['color'] = colors_curve
        df_curves['x0'] = df_curves['signed_contrast'] - 5
        df_curves['x1'] = df_curves['signed_contrast'] + 5
        
        df_curves['contrast_axis'] = [contrast_map[c] for c in df_curves['signed_contrast']]
    ...
    def plot(self):
        f =figure(title="",width=450,height=400,y_axis_type='log',
                  x_axis_label='Contrast(%)', y_axis_label='Response Time(ms)',
                  x_range=(-3, 3))
        
        f.vspan(x=0,line_dash='dashed',color='#000000',line_width=3)
        f.hspan(y=1000,line_dash='dashdot',color='#f00000',line_width=3)
        
        f.circle(x=jitter('contrast_axis',width=0.3,distribution='uniform'), y='reaction_time',
                 size=10,color='color',source=self.cds_dots,alpha=0.6)
        
        f.segment(x0='x0_axis', y0='median_reaction_time',
                  x1='x1_axis', y1='median_reaction_time',
