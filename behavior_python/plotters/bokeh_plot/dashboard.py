...
class DashBoard:
    ...
    def set_animal_sessions(self) -> None:
        ...
        path2look = pjoin(self.config[path_key],f"*_{self.current_animal}_detect_*",)
        self.session_list = [s.split(os.sep)[-1] for s in glob.glob(path2look)] # strip the J:\\presentation(or training)
    ...
    def set_session(self, session:str=None) -> None:
        ...
        try:
            session_obj = WheelDetectionSession(sessiondir=self.current_session,load_flag=True)
        except:
            session_obj = WheelDetectionSession(sessiondir=self.current_session,load_flag=False)
