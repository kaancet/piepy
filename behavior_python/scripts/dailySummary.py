    ...
    def init_day(self):
        if self.day is None:
            ...
            try:
                ...
                raise ValueError('Date string not recognized, format should be YYMMDD, e.g. 210926')
...
def dailySummary(day:str=None,to_plot:list=None) -> None:
    ...
    display('Producing daily summaries {0} {1}'.format(today.day_nice_str,today.day_of_week))
    ...
    today_experiments = glob.glob('{0}/{1}*__no_cam_*/'.format(presentation_folder,today.day))
    ...
    row_length = 5
    col_width = 8
    fig = plt.figure(figsize=(int(col_width*len(plot_types)),
                              int(row_length*len(to_plot))))

    for row,animalid in enumerate(to_plot):
        # columns for different plot panels
        for col,pt in enumerate(plot_types):
            ax = fig.add_subplot(len(to_plot),len(plot_types),int(col+row*(len(plot_types)))+1)
            analysed_wheel[animalid].plot(pt, ax=ax, savefig=False, notitle=True)
            
            if col == 0:
                ax.annotate(animalid,
                            xy=(0, 0.5), 
                            xytext=(0.1, 0),
    ...
    fig.savefig('J:\\data\\analysis\\behavior_results\\python_figures\\dailyPlots\\{0}_summary.pdf'.format(today.day),dpi=100,bbox_inches='tight')
...
def main():
    ...
    parser.add_argument('day',metavar='summaryday',
                        type=str,help='Day to be summarized (e.g. 191124)')
