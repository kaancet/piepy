...
class TrainingWeek():
    ...
    def init_week(self) -> None:
        ...
        try:
            ...
            raise ValueError(f'Date string {self.day_end} not recognized, use YYMMDD, e.g. 220918')
    ...
    def get_session(self,date:str) -> list:
        ...
        if not len(sesh):
            ...
            if not len(sesh):
                display(f'No session in training directory, looking at presentation folder')
                sesh = glob.glob(f'J:\\presentation\\{date}_{self.animalid}_{self.task_type}*')
        ...
        for i,sesh in enumerate(self.session_list):
            ...
            for k,v in temp.data.stim_data.items():
                if k not in self.week_data['week_pooled'].keys():
                    ...
                    self.week_data['week_pooled'][k] = pd.concat([self.week_data['week_pooled'][k],v])
