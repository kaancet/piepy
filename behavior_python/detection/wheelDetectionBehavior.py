...
class WheelDetectionBehavior(Behavior):
    def __init__(self, animalid: str, load_type: str = None, *args, **kwargs):
        ...
        )  # this is a failsafe to get only detect sessions
    ...
    def get_behavior(self):
        ...
        if len(missing_sessions) == len(self.session_list):
            ...
            cumul_data = pl.read_parquet(
                pjoin(
                    self.analysisfolder,
                    self.cumul_file_loc,
                    "detectTrainingData.parquet",
                )
            )
            ...
            if len(session_data):
                ...
                if i == 0:
                    ...
                    try:
                        ...
                        session_data = session_data.with_columns(
                            pl.col("opto").cast(pl.Int64).alias("opto")
                        )
                        cumul_data = cumul_data.with_columns(
                            pl.col("opto_region").cast(str).alias("opto_region")
                        )
                        session_data = session_data.with_columns(
                            pl.col("opto_region").cast(str).alias("opto_region")
                        )
...
def main():
    ...
    parser.add_argument(
        "id", metavar="animalid", type=str, help="Animal ID (e.g. KC020)"
    )
    parser.add_argument(
        "-d",
        "--date",
    ...
    w = WheelDetectionBehavior(
        animalid=animalid, dateinterval=dateinterval, criteria=criteria
    )
