...
class WheelDetectionExperiment:
    ...
    def parse_sessions(self, load_sessions: bool = False) -> pl.DataFrame:
        ...
        for i, exp in enumerate(pbar):
            ...
            lit = {
                ...
                "opto_targets": len(
                    nonan_unique(w.data.data["opto_pattern"].to_numpy())
                )
                - 1,
            ...
            if i == 0:
                ...
                temp_df = temp_df.with_columns(
                    pl.lit(i + 1).cast(pl.Int32).alias("session_no")
                )
                list_df = pl.DataFrame(non_lit)
    ...
    def make_summary_data(self) -> pl.DataFrame:
        ...
        q = (
            self.data.lazy()
            .groupby(
                ["animalid", "area", "stimulus_count", "opto_targets", "isTitrated"]
            )
            .agg(
                [
    ...
    def filter_by_animal(
        ...
        filt_df = self.filter_by_params(
            filt_summ, stim_count, stim_type, opto_targets, isTitrated, verbose
        )
        return filt_df
