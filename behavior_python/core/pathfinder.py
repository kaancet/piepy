...
class PathFinder:
    ...
    def init_directories_from_config(self) -> None:
                    ...
                    if exists(session_related_dir):
                        ...
                        if found_runs is None:
                            ...
                            if name in ['training','presentation']:
                                tmp_runs  = found_runs
                            self.all_paths[name] = [pjoin(session_related_dir,i) for i in found_runs]
        ...
        if self.all_paths['presentation'] is None and self.all_paths['training'] is None:
            raise FileNotFoundError(f"{self.sessiondir} does not exist in neither presentation or training!!")
        elif self.all_paths['presentation'] is not None and self.all_paths['training'] is not None:
            raise FileExistsError(f"{self.sessiondir} exists both in presentation and training!!")
