...
class BehaviorData:
    ...
    def filter_dates(self, dateinterval: list):
        ...
        if isinstance(dateinterval, str):
            ...
            assert (
                len(dateinterval) <= 2
            ), f"You need to provide a single start(1) or start and end dates(2), got {len(dateinterval)} dates"
        ...
        self.summary_data["dt_date"] = self.summary_data["date"].apply(
            lambda x: dt.strptime(str(x), "%y%m%d")
        )
        self.cumul_data["dt_date"] = self.cumul_data["date"].apply(
            lambda x: dt.strptime(str(x), "%y%m%d")
        )

        self.summary_data[
            (self.summary_data["dt_date"] >= startdate)
            & (self.summary_data["dt_date"] <= enddate)
        ]
        self.cumul_data[
            (self.cumul_data["dt_date"] >= startdate)
            & (self.cumul_data["dt_date"] <= enddate)
        ]
    ...
    def save(self, path: str, task_type: str) -> None:
        ...
        summary_save_data = self.summary_data.with_columns(
            [
                ("[" + pl.col("sf").cast(pl.List(pl.Utf8)).list.join(", ") + "]").alias(
                    "sf"
                ),
                ("[" + pl.col("tf").cast(pl.List(pl.Utf8)).list.join(", ") + "]").alias(
                    "tf"
                ),
            ]
        )
...
class Behavior:
    ...
    def get_sessions(self) -> list:
        ...
        experiment_sessions = glob.glob(
            "{0}/*{1}*__no_cam_*/".format(self.presentationfolder, self.animalid)
        )
        onep_sessions = glob.glob(
            "{0}/*{1}*__1P_*/".format(self.presentationfolder, self.animalid)
        )
        twop_sessions = glob.glob(
            "{0}/*{1}*__2P_*/".format(self.presentationfolder, self.animalid)
        )
        training_sessions = glob.glob(
            "{0}/*{1}*__no_cam_*/".format(self.trainingfolder, self.animalid)
        )
    ...
    def read_googlesheet(self) -> None:
        ...
        self.gsheet_df["weight [g]"] = self.gsheet_df["weight [g]"].apply(
            lambda x: str(x).replace(",", ".")
        )
        self.gsheet_df["weight [g]"] = pd.to_numeric(
            self.gsheet_df["weight [g]"], errors="coerce"
        )
        self.gsheet_df["supp water [µl]"] = pd.to_numeric(
            self.gsheet_df["supp water [µl]"]
        ).fillna(0)

        self.gsheet_df["Date [YYMMDD]"] = self.gsheet_df["Date [YYMMDD]"].apply(
            lambda x: str(x)
        )
        self.gsheet_df["Date_dt"] = pd.to_datetime(
            self.gsheet_df["Date [YYMMDD]"], format="%y%m%d"
        )
    ...
    def get_non_data(self):
        ...
        temp = self.gsheet_df[
            self.gsheet_df["paradigm"].isin(
                ["handling", "water restriction start", "surgery"]
            )
        ]
    ...
    def save_data(self, task_type: str) -> None:
        ...
        if (
            not self.behavior_data.summary_data.is_empty()
            and not self.behavior_data.cumul_data.is_empty()
        ):
                ...
                if self.cumul_file_loc != latest_session:
                    ...
                    os.remove(
                        pjoin(
                            self.analysisfolder,
                            self.cumul_file_loc,
                            f"{task_type}TrainingData.parquet",
                        )
                    )
                    os.remove(
                        pjoin(
                            ...
                            f"{task_type}TrainingDataSummary.csv",
                        )
                    )
    ...
    def isSaved(self, task_type: str) -> bool:
        ...
        cumul_data_saved_loc = glob.glob(
            f"{self.analysisfolder}/*{self.animalid}*/{task_type}TrainingData.parquet"
        )
        summary_data_saved_loc = glob.glob(
            f"{self.analysisfolder}/*{self.animalid}*/{task_type}TrainingDataSummary.csv"
        )

        if len(cumul_data_saved_loc) > 1 and len(summary_data_saved_loc) > 1:
            display(
                f"There should be only single _trainingData.parquet (most recent one) found {summary_data_saved_loc}"
            )
