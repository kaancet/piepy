...
class DataBaseInterface:
    ...
    def update_entry(
        self, entry: dict, update_dict: dict, table_name: str, verbose: bool = True
    ) -> None:
    ...
    def remove_entry(self, entry: dict, table_name: str) -> None:
        ...
        if isinstance(entry, dict):
            if len(entry) > 1:
                raise ValueError(
                    f"Entry search should be done with a single key:value pair dictionary"
                )
            entry_field, entry_val = zip(*entry.items())
        elif isinstance(entry, str):
            if entry != "all":
                raise ValueError(
                    f'{entry} is not a valid value for entry use a dict or "all" to delete everyhting in the table'
                )
        else:
    ...
    def print_table(self, table_name: str) -> pd.DataFrame:
        ...
        for entry in self.cursor.execute(f"SELECT * FROM {safe_str(table_name)}"):
            table_df = table_df.append(
                pd.Series(entry, index=col_names), ignore_index=True
            )
        self.close()
        return table_df
