...
def main():
    ...
    parser.add_argument('sessiondir',metavar='sessiondir',
                        type=str,help='Directry of the session, e.g. 231103_KC143_detect__no_cam_KC')
    parser.add_argument('-p','--paradigm',metavar='paradigm',
                        type=str,help='Behavior paradigm(e.g. detection)')
    parser.add_argument('-l','--load',default=True,metavar='load_type',action=argparse.BooleanOptionalAction,
                        type=str,help='Loading parameter')
    ...
    mod_path = normpath(pjoin(abspath(dirname(dirname(__file__))),opts.paradigm,session_class_name+'.py'))
    if exists(mod_path):
        mod = importlib.import_module(f'behavior_python.{opts.paradigm}.{session_class_name}')
