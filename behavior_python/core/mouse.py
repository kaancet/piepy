...
class Mouse:
    ...
    def update_field(self,field:str,value) -> None:
        ...
        if not entry.empty:
            ...
            self.db_interface.update_entry({'id': self.id},
                                           {field: value},
                                           'animals')
        else:
    ...
    def read_gsheet(self) -> pd.DataFrame:
        ...
        sheet_df['weight [g]'] = sheet_df['weight [g]'].apply(lambda x: str(x).replace(',','.'))
        sheet_df['weight [g]'] = pd.to_numeric(sheet_df['weight [g]'],errors='coerce')
        sheet_df['supp water [µl]'] = pd.to_numeric(sheet_df['supp water [µl]']).fillna(0)
        
        sheet_df['Date [YYMMDD]'] = sheet_df['Date [YYMMDD]'].apply(lambda x: str(x))
        sheet_df['Date [YYMMDD]'] = pd.to_datetime(sheet_df['Date [YYMMDD]'],format='%y%m%d')
