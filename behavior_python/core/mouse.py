...
class MouseData:
    ...
    def append(self,cumul_data_list:list,summary_data_list:list) -> None:
        ...
        assert len(cumul_data_list)==len(summary_data_list['date']), f'Cumulative and summary data has to be the same length!!'
    ...
    def save(self,save_path:str) -> None:
        ...
        if self.cumul_data is not None:
            cumul_save_name = pjoin(save_path,f'{self.paradigm}BehaviorData.parquet').replace("\\","/")
            summary_save_name = pjoin(save_path,f'{self.paradigm}BehaviorDataSummary.csv').replace("\\","/")
    ...
    def load(self,load_path:str) -> None:
        ...
        self.cumul_data = pl.read_parquet(pjoin(load_path,f'{self.paradigm}BehaviorData.parquet'))
        
        summary_data = pl.read_csv(pjoin(load_path,f'{self.paradigm}BehaviorDataSummary.csv'))        
        # sf and tf needs to be converted back to lists
        self.summary_data = summary_data.with_columns([pl.col("sf")
...
class Mouse:
    ...
    def set_paradigm(self,paradigm:str) -> None:
        ...
        if self.paradigm is not None:
            ...
            self.session_list = self.all_sessions.filter(pl.col('paradigm')==self.paradigm)
    ...
    def get_sessions(self) -> pl.DataFrame:
        ...
        training_sessions = glob.glob(f'{self.paths.training}/*{self.animalid}*__no_cam_*/')
        ...
        session_list = pl.DataFrame(data={'animalid':[self.animalid]*len(all_sessions),
                                          'sessiondir':sessions,
                                          'date':dates,
                                          'exp_type':types,
                                          'fullpath':all_sessions})
        
        session_list = session_list.with_columns(pl.when(pl.col('sessiondir').str.contains('detect'))
                                                 .then(pl.lit('detection'))
                                                 .otherwise(None).alias('paradigm'))
    ...
    def gather_data(self,load_type:str=None) -> None:
        ...
        if self.paradigm is None:
            display('No paradigm set to analyze the data, do that first by using set_paradigm method',color='orange')
            return None
        
        if load_type not in self.load_modes:
            raise ValueError(f'{load_type} is not a valid loading mode, try one of: {self.load_modes}')
        ...
        for i,row in enumerate(missing_sessions.iter_rows()):
            ...
            if load_type == 'no_load':
                detect_session = self.session_parser(row[1],load_flag=False, skip_google=True)
            else:
                # reanalyze load type should enter here
                detect_session = self.session_parser(row[1],load_flag=True, skip_google=True)

            session_data = detect_session.data.data
            gsheet_dict = self.get_gsheet_row(detect_session.meta.baredate,
    ...
    def read_googlesheet(self) -> None:
        ...
        temp_df['weight [g]'] = temp_df['weight [g]'].apply(lambda x: str(x).replace(',','.'))
        temp_df['weight [g]'] = pd.to_numeric(temp_df['weight [g]'],errors='coerce')
        temp_df['supp water [µl]'] = pd.to_numeric(temp_df['supp water [µl]'],errors='coerce').fillna(0)
        ...
        if not self.data.summary_data.is_empty():
            if self.saved_dir is not None:
                ...
                if self.saved_dir != latest_session:
                    ...
                    os.remove(pjoin(del_target,f'{self.paradigm}BehaviorDataSummary.csv'))
    ...
    def isSaved(self) -> bool:
        ...
        cumul_data_saved_loc = glob.glob(f'{self.paths.analysis}/*{self.animalid}*/{self.paradigm}BehaviorData*.parquet')
        summary_data_saved_loc = glob.glob(f'{self.paths.analysis}/*{self.animalid}*/{self.paradigm}BehaviorDataSummary.csv')
        
        if len(cumul_data_saved_loc) > 1 and len(summary_data_saved_loc) > 1:
            display(f'There should be only single _trainingData.parquet (most recent one) found {summary_data_saved_loc}, using the last one...',color='yellow')
    ...
    def get_unanalyzed_sessions(self,load_type:str) -> pl.DataFrame:
        ...
        if load_type == 'load_and_add':
            ...
            if not is_saved:
                # no saved file found!
                raise FileNotFoundError(f"Can't do load, there is no saved behavioral analysis files for cumulative (*.parquet) and summary (*.csv) data")
    ...
    def get_session_class(session_type:str) -> None:
        ...
        mod_path = normpath(pjoin(abspath(dirname(dirname(__file__))),session_type,session_class_name+'.py'))
        if os.path.exists(mod_path):
            mod = importlib.import_module(f'behavior_python.{session_type}.{session_class_name}')
            session_class_name = session_class_name[0].upper() + session_class_name[1:] # uppercasing the first letter for class name
