...
class WheelTrial(Trial):
    ...
    def get_vstim_props(self) -> dict:
        ...
        vstim_dict['stim_pos'] = vstim_dict['posx_r'] if vstim_dict['correct'] else vstim_dict['posx_l']
        ...
        try:
            ...
            display(f' >>>>> WARNING <<<<< Trial {self.trial_no} has an issue: {vstim_dict["stim_pos"]}')
    ...
    def get_wheel_pos(self,anchor:float) -> np.ndarray:
        ...
        if len(wheel_arr):
            ...
            wheel_arr[:,0] = np.apply_along_axis(lambda x: x-anchor,0,wheel_arr[:,0])
    ...
    def trial_data_from_logs(self) -> list:
        ...
        for _,row in self.data['state'].iterrows():
            ...
            # incorrect
                ...
                if trial_log_data['response_latency']>=float(self.meta.closedStimDuration)*1000:
            ...
            # correction or trial end
                ...
                # No go is 0
                trial_log_data['choice'] = int(np.sign(trial_log_data['stim_side']) * trial_log_data['answer'])
    ...
    def save_trial_db(self,**kwargs):
        ...
        temp_dict = {}
        ignore = ['correct','trial_no','wheel','stim_pos','posx_l','posx_r','opto_pulse','lick']
