...
class WheelCurve:
    ...
    __slots__ = ['data','fit_model','fit_side','contrast','signed_contrast',
                 'percentage','confidence','counts','fit_pars','fitted_curve','bias']
    def __init__(self,data:pd.DataFrame=None,fit_model:str='erf_psycho2',fit_side:str='right',init_dict:dict=None):
    ...
    def fit_curve(self,**kwargs):
        ...
        x_axis = np.linspace(np.min(self.signed_contrast),
                             np.max(self.signed_contrast),
                             kwargs.get('resolution',100)).reshape(-1, 1)
        
        if self.fit_model == 'naive':
            try:
                popt,pcov = naive_fit(self.signed_contrast,
                                      self.percentage,**kwargs)

                self.fit_pars = {'pars' : popt,
                                 'pcov' : pcov}
                
                y_axis = np.asarray(sigmoid(x_axis, *self.fit_pars['pars'])).reshape(-1, 1)
                
            except:
                display('Failed to fit to data')
                self.fit_pars= {'pars' : 0,
                                'popt' : 0}
                x_axis = self.signed_contrast.reshape(-1,1)
                y_axis = self.percentage.reshape(-1,1)

        elif self.fit_model == 'erf_psycho2': 
            data = np.vstack((self.signed_contrast,
                              self.counts,
                              self.percentage))
            # try:
            fit_pars, fit_L = mle_fit(data,self.fit_model,side=self.fit_side,nfits=kwargs.get('nfits',10))

            self.fit_pars = {'pars' : fit_pars,
                             'likelihood' : fit_L}
        ...
        for i,_ in enumerate(self.analysis[analyz]['data_types']):
            for j in range(1,len(self.analysis[analyz]['data_types'])):
                try:
                    ...
                    self.pairs[key] =[self.analysis[analyz]['data_types'][i],self.analysis[analyz]['data_types'][i+j]]
                except:
                    pass
        ...
        for i,p_key in enumerate(self.pairs.keys()):
            ...
            if 'mantel_haenzsel' in analyz:
                ...
                for j,typ in enumerate(pair):
                    for k,contrast in enumerate(self.analysis['summary'][typ]['sided_contrast_list']):
                        ...
                        if j == 0:
                            self.analysis[analyz][p_key]['data_table'][contrast] = table_row
                        else:
                            self.analysis[analyz][p_key]['data_table'][contrast] = np.vstack((self.analysis[analyz][p_key]['data_table'][contrast],
                                                                                                table_row))

                display('Running analysis {0} ...'.format(p_key))
                self.analysis[analyz][p_key]['results'],_ = mantel_haenzsel(data=self.analysis[analyz][p_key]['data_table'],stats=analyz_arg)
...
class WheelModel:
    ...
    def model_data(self,data:pd.DataFrame):
        if not isinstance(data,pd.DataFrame):
            raise TypeError(f'Data needs to be a DataFrame [trials x features], got {type(data)} instead')
    ...
    def fit_model(self,seed_init:int=65):
        ...
        if self.input_matrix is None and self.y is None:
            raise ValueError('Input matrix and the output(y) has to be set before fitting the model')
