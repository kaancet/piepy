...
class WheelSessionPlotter(WheelBasePlotter):
    ...
    def save(self,plotkey):
        ...
        savename = '{0}_{1}_{2}.html'.format(self.session['meta']['baredate'],
    ...
    def set_scope(self,scope='all'):
        ...
        if scope == 'novel': 
            ...
            if data is None:
                raise ValueError('Data scope {0} does not exist, try one of {1}'.format(list(scope,self.session['novel_stim_data'].keys())))
        return data
        ...
        if filters is None:
            ...
            if 'trial_limit' in filters.keys():
                if isinstance(data,dict):
                    filtered_data = {k: v[v['trial_no'] <= filters['trial_limit']] for k,v in data.items()}
                else:
    ...
    def psychometric(self, mode='src',fig=None, *args, **kwargs):
        ...
        for key,d in data.items():
            ...
            fig.line(x=100 * curve.contrast_x_fitted[:,0],y=fit_y,
                    line_color=area_colors[key],
                    level='underlay',
        ...
        if not kwargs.get('notitle',False):
            fig.title.text = '{0} Psychometric Curve {1}'.format(self.session['meta']['animalid'],self.session['meta']['date'])
    ...
    def performance(self, fig=None, *args, **kwargs):
        ...
        # plotting
        source = ColumnDataSource(data={'time_in_secs' : data['openstart_absolute'] / 60000,
                                        'performance_percent' : data['fraction_correct'] * 100})

        fig.line(x='time_in_secs', y='performance_percent',
                 line_color='#148428',
        ...
        # some logic for embedding in summary figures
        if not kwargs.get('notitle',False):
            fig.title.text = '{0} Performance {1}'.format(self.session['meta']['animalid'],self.session['meta']['date'])
            fig.title.text_font_size = self.pt_font(fontsize+2)
            fig.title.text_font_style = 'bold'
    ...
    def responseLatency(self, sep_sides=False, fig=None, *args, **kwargs):
        ...
        if fig is None:
            fig = bok.figure(toolbar_location=kwargs.get('toolbarlocation','above'),
                             tools = "pan,box_zoom,reset", y_axis_type='log')
        ...
        for key in data.keys():
            ...
            source_line = ColumnDataSource(data = {'running_response_latency' : data[key]['running_response_latency']/1000,
                                                   'trial_no' : data[key]['trial_no']})

            fig.line(x='trial_no', y='running_response_latency',
            ...
            if kwargs.get('plottrials',True):
                source_scatter = ColumnDataSource(data = {'trial_no' : data[key]['trial_no'],
                                                          'response_latency' : data[key]['response_latency']/1000})
                fig.circle(x='trial_no',y='response_latency',
        ...
        # some logic for embedding in summary figures
        if not kwargs.get('notitle',False):
            fig.title.text = '{0} Response Time {1}'.format(self.session['meta']['animalid'],self.session['meta']['date'])
            fig.title.text_font_size = self.pt_font(fontsize+2)
            fig.title.text_font_style = 'bold'
    ...
    def probability(self, fig=None, *args, **kwargs):
        ...
        fig.line(time_left,probs_left,
                 line_color='#BA2740',
                 line_width=kwargs.get('linewidth',5),
                 legend_label='Left')

        fig.line(time_right,probs_right,
                 line_color='#34B8BA',
                 line_width=kwargs.get('linewidth',5),
                 legend_label='Right')
        ...
        # some logic for embedding in summary figures
        if not kwargs.get('notitle',False):
            fig.title.text = '{0} Stim Side Probabilities {1}'.format(self.session['meta']['animalid'],self.session['meta']['date'])
            fig.title.text_font_size = self.pt_font(fontsize+2)
            fig.title.text_font_style = 'bold'
    ...
    def parameterCompare(self, p1, p2, fig=None, *args, **kwargs):
        ...
        source_fit = ColumnDataSource(data={'fit_x':data[p1],
                                            'fit_y':fit_line})

        source_scatter = ColumnDataSource(data={'x':data[p1],
                                                'y':data[p2]})
        ...
        # some logic for embedding in summary figures
        if not kwargs.get('notitle',False):
            fig.title.text = '{0} {1}'.format(self.session['meta']['animalid'],self.session['meta']['date'])
            fig.title.text_font_size = self.pt_font(fontsize+2)
            fig.title.text_font_style = 'bold'
    ...
    def parameterFractions(self, param_list, fig=None, *args, **kwargs):
        ...
        if kwargs.get('drawtrials',True):
            ...
            fig.multi_line(xs='trial_no',ys='trial_line',
                     line_width=1.3,
                     line_color='color',
                     line_alpha=0.3,
                     source=lines_source)
        ...
        fig.multi_line(xs='trial_no',ys='params',
                      line_color='color',
    ...
    def metricSummary(self, *args, **kwargs):
        ...
        ax1 = self.metric('path_surplus',ax=ax1,notitle=True,savefig=False,*args,**kwargs)

        ax2 = self.fig.add_subplot(222)
        ax2 = self.metric('reaction_t',ax=ax2, notitle=True,savefig=False,*args,**kwargs)

        ax3 = self.fig.add_subplot(223)
        ax3 = self.metric('avg_lick_t_diff',ax=ax3, notitle=True,savefig=False,*args,**kwargs)

        ax4 = self.fig.add_subplot(224)
        ax4 = self.metric('avg_speed',ax=ax4, notitle=True,savefig=False,*args,**kwargs)
    ...
    def sessionSummary(self, *args, **kwargs):
        ...
        widths = [1, 1.5, 2]
        heights = [1,1,1]
        gs = self.fig.add_gridspec(ncols=3, nrows=3, width_ratios=widths,
        ...
        ax_perf = self.parameterFractions(param_list=['fraction_correct','fraction_nogo'],
                                          ...
                                          ax=ax_perf,
                                          savefig=False, notitle=True, showlegend=False,*args,**kwargs)
        h_perf,l_perf = ax_perf.get_legend_handles_labels()

        # total licks
        ax_cumu = ax_perf.twinx()
        ax_cumu = self.lickTotal(ax=ax_cumu,
                                 savefig=False, notitle=True, showlegend=False,*args,**kwargs)
        ...
        ax_frac = self.parameterFractions(param_list=['fraction_repeat','fraction_stim_right'],
                                               colors=['olive','teal'], 
                                               ax=ax_frac,
                                               drawtrials=False, savefig=False, notitle=True, showlegend=False,*args,**kwargs)
        h_frac,l_frac = ax_frac.get_legend_handles_labels()

        # response latency
        ax_time = ax_frac.twinx()
        ax_time = self.responseLatency(sep_sides=False,
                                       ax=ax_time, savefig=False, notitle=True, showlegend=False,*args,**kwargs)
        ...
        ax_hist = self.lickHistogram(ax=ax_hist, notitle=True, savefig=False,*args,**kwargs)
        
        # wheel
        ax_wheel = self.fig.add_subplot(gs[1:,2])
        ax_wheel = self.wheelTrajectory_vert(ax=ax_wheel, notitle=True, savefig=False,*args,**kwargs)
