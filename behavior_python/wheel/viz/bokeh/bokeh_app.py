from behavior_python.wheel.viz.bokeh.bokeh_plotting import WheelTrace
from bokeh.io import curdoc
from bokeh.layouts import column,row,gridplot,layout
from bokeh.models import ColumnDataSource, Slider, RangeSlider, Select, CheckboxGroup, Panel, Tabs, Div,FileInput, Spinner

from bokeh_plotting import DashBoard,Graph,Performance,ResponseTime,Psychometric,TrialPicture, ContrastDistribution, AnswerDistribution, WheelTrace
...
from behavior_python.wheel.wheelBehavior import *
...
for animal in animal_list:
    ...
    main_session_dict[animal] = [s.split(splitter)[-2] for s in glob.glob('{0}/*_{1}_*_{2}/'.format(analysis_folder,animal,'KC'))]
...
dash.add_widget('session_selector',
                Select(title='Select Session', options=main_session_dict[dash.current_animal],value=dash.current_session))
# trial count slider
dash.add_widget('trial_count_slider',
                RangeSlider(start=1, end=dash.shown_data[dash.current_scope]['trial_no'].iloc[-1], 
                            value=(dash.shown_data[dash.current_scope]['trial_no'].iloc[0], dash.shown_data[dash.current_scope]['trial_no'].iloc[-1]), 
                            step=1, title='Trial Count Filter'))
# response time filter
dash.add_widget('response_time_slider',
                Slider(start=200,end=5000,value=200,step=100,title='Response Latency Filter(ms)'))
# trial no slider
dash.add_widget('trial_no_spinner',
                Spinner(low=1,high=dash.shown_data[dash.current_scope]['trial_no'].iloc[-1],width=100,
                value=dash.shown_trial['trial_no'],step=1,title='Trial No'))
#scope selector
dash.add_widget('scope_selector',
                Select(title='Scope of Data',options=dash.scope_list,value=dash.current_scope,width=150))
# novelity
...
dash.add_widget('time_axis',
                CheckboxGroup(labels=['Toggle Time x-Axis'],active=[int(dash.time_axis)]))
#wheel individual
dash.add_widget('show_individual',
                CheckboxGroup(labels=['Show Individual Wheel Traces'],active=[int(dash.show_individual)]))

# meta and stats
dash.add_widget('meta_div',
                Div(text=dash.meta_data,style={'font-size':'15px'}))

dash.add_widget('stats_div',
                Div(text=dash.stats[dash.current_scope],style={'font-size':'15px'}))
...
def session_selector_callback(attr,old,new):
    ...
    if val != '':
        ...
        dash.widgets['trial_count_slider'].end = dash.shown_data[dash.current_scope]['trial_no'].iloc[-1]
...
tab_contrast = Panel(child = dash.graphs['contrastDistribution'].fig, title='Contrast Distribution')
tab_answer = Panel(child = dash.graphs['answerDistribution'].fig, title = 'Answer Distribution')

animal_sesh_ctrl_box = row(dash.widgets['animal_selector'],dash.widgets['session_selector'])
sesh_ctrl_info_box = column(dash.widgets['trial_count_slider'],
                            dash.widgets['scope_selector'],
                            dash.widgets['response_time_slider'],
                            dash.widgets['is_novel'],
                            Tabs(tabs=[tab_meta,tab_stats]))

perf_resp_tab_box = column(row(dash.widgets['time_axis']),
                           Tabs(tabs=[tab_perf,tab_time]))

distributions_tab_box = column(Tabs(tabs=[tab_contrast,tab_answer]))

trial_pic_box = column(row(dash.widgets['trial_no_spinner']),
                       dash.graphs['trialPicture'].fig)

wheel_trace_box = column(row(dash.widgets['show_individual']),
                         dash.graphs['wheelTrace'].fig)

layout = column(main_title,
                animal_sesh_ctrl_box,
                row(sesh_ctrl_info_box,
                    column(perf_resp_tab_box,
                           row(distributions_tab_box,dash.graphs['psychometric'].fig)
                           ),
                    trial_pic_box,
                    wheel_trace_box
                    )
