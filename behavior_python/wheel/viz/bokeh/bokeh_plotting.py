...
class DashBoard:
    ...
    def init_data(self,data_in):
        ...
        self.scope_list = [k for k in self.data['novel_stim_data'].keys() if k!='overall']
    ...
    def set_data(self):
        ...
        if self.data is not None:
            self.shown_data = copy.deepcopy(self.data)
            if self.show_repeat:
                self.shown_data = {self.current_scope : self.shown_data['data'][self.current_scope]}
            else:
                self.shown_data = {self.current_scope : self.shown_data['novel_stim_data'][self.current_scope]}
    ...
    def change_scope(self,scope):
        ...
        if self.scope_list is not None:
            if scope in self.scope_list:
                ...
                raise ValueError('Data scope {0} does not exist, try one of {1}'.format(scope,self.scope_list))
            ...
            if self.data is not None:
                if 'trial_limit' in filters.keys():
                    ...
                    if isinstance(self.shown_data,dict):
                        self.shown_data = {k: v[v['trial_no'] <= filters['trial_limit']] for k,v in self.shown_data.items()}
                    else:
                        self.shown_data = self.shown_data[self.shown_data['trial_no'] <= filters['trial_limit']]
                elif 'response_cutoff' in filters.keys():
                    self.shown_data = copy.deepcopy(self.data)
                    if isinstance(self.shown_data,dict):
                        self.shown_data = {self.shown_data[k]: v[v['response_latency'] <= filters['response_cutoff']] for k,v in self.shown_data.items()}
                    else:
                        self.shown_data = self.shown_data[self.shown_data['response_latency'] <= filters['response_cutoff']]
...
class Psychometric(Graph):
    ...
    def plot(self,*args,**kwargs):
        ...
        fig = bok.figure(toolbar_location=kwargs.get('toolbarlocation','right'),
                            tools = "pan,box_zoom,wheel_zoom,reset",plot_width=500, plot_height=500)
        ...
        for scope in self.cds_dots.keys():
            ...
            fig.circle(x='contrast_x',y='prob',
            ...
            fig.add_layout(
                Whisker(source=self.cds_dots[scope],base='contrast_x',upper='err_up',lower='err_down',
        ...
        fig.add_tools(HoverTool(names=['points'],
                                tooltips=[('Trial Count','@trial_count'),
                                          ('%95','@err')],
                                mode='mouse'))
    ...
    def set_cds(self,data,**kwargs):
        ...
        for scope,data in fitted_data.items():
            ...
            temp_dots  = {'contrast_x' : 100 * np.array(data[self.side]['contrast']),
                        'prob' : data[self.side]['percentage'],
                        'err': data[self.side]['confidence'],
                        'err_up': [x+e for x,e in zip(data[self.side]['percentage'],data[self.side]['confidence'])],
                        'err_down': [x-e for x,e in zip(data[self.side]['percentage'],data[self.side]['confidence'])],
            ...
            temp_curves = {'contrast_x':100 * data[self.side]['fitted_x'],
...
class Performance(Graph):
    ...
    def plot(self,*args,**kwargs):
        ...
        fig = bok.figure(toolbar_location=kwargs.get('toolbarlocation','above'),
                     tools = "pan,box_zoom,wheel_zoom,reset",plot_width=800, plot_height=400)
        
        for scope in self.cds.keys():
            fig.line(x='x_axis', y='performance_percent',
                     line_color=stim_styles[scope],
                     line_width=kwargs.get('linewidth',5),
                     source=self.cds[scope],
                     legend_label='Accuracy(%)')

            fig.multi_line(xs='_x_axis',ys='_x_axis_line',
        ...
        # add hover tool
        fig.add_tools(HoverTool(names=['trials'],
                                tooltips=[('Trial No/Time','@x_axis'),
                                          ('Side','@_trial_side'),
    ...
    def set_cds(self,data,**kwargs):
        ...
        for i,scope in enumerate(data.keys()):
            ...
            if kwargs.get('drawtrials',True):
                ...
                temp_trials = {'_trial_color' : d['trial_color'],
                               '_trial_side' : d['stim_side'],
                               '_trial_contrast' : d['contrast']}
            else:
                temp_trials = {'_trial_no':[[]],
                               '_trial_line' :[[]],
                               '_trial_color':[],
                               '_trial_side':[],
                               '_trial_contrast':[]}
...
class ResponseTime(Graph):
    ...
    def plot(self,*args,**kwargs):
        fontsize = kwargs.get('fontsize',14)
        # set up axes
        fig = bok.figure(y_axis_type='log',toolbar_location=kwargs.get('toolbarlocation','above'),
                        tools = "pan,box_zoom,wheel_zoom,reset",plot_width=800, plot_height=400)

        for scope in self.cds.keys():
            fig.line(x='x_axis', y='running_response_latency',
                     line_color=stim_styles[scope],
                     line_width=kwargs.get('linewidth',5),
                     source=self.cds[scope])

            if kwargs.get('plottrials',True):
                fig.circle(x='x_axis',y='response_latency',
                           size=7,
                           fill_color=stim_styles[scope],
    ...
    def set_cds(self,data,**kwargs):
        ...
        for i,key in enumerate(data.keys()):
            ...
            temp = {'running_response_latency' : d['running_response_latency'] / 1000,
...
class AnswerDistribution(Graph):
    ...
    def plot(self, *arg, **kwargs):
        ...
        # set up axes
        fig = bok.figure(x_range=FactorRange(*self.x_axis_types),toolbar_location=kwargs.get('toolbarlocation','above'),
                        tools = "pan,box_zoom,reset",plot_width=400, plot_height=400)

        for scope in self.cds.keys():
            fig.x_range = FactorRange(*self.cds[scope].data['x'])
            fig.vbar_stack(self.answers[scope],x='x',
                           width=0.9,
                           color=self.colors,
                           source=self.cds[scope],
        ...
        # add hover tool
        fig.add_tools(HoverTool(tooltips=[('Count','$y')],
                                mode='mouse'))
...
class ContrastDistribution(Graph):
    ...
    def plot(self,*args,**kwargs):
        ...
        for scope in self.cds.keys():
            fig.vbar_stack(self.contrasts[scope],x='x',
                           width=0.9,
                           color=self.colors,
                           source=self.cds[scope],
...
class TrialPicture(Graph):
    ...
    def plot(self,**kwargs):
        ...
        # set up axes
        fig = bok.figure(toolbar_location=kwargs.get('toolbarlocation','above'),
                        tools = "pan,box_zoom,wheel_zoom,reset",plot_width=800, plot_height=500)

        # static lines and zones
        # stim on screen 
        fig.harea(x1='stimstart',x2='stimend',y='dummy_vert_line',
                  fill_color='#bcbcbc',
                  fill_alpha=0.5,
        ...
        # decision
        fig.harea(x1='closedloopstart',x2='closedloopend',y='dummy_vert_line',
                  fill_color='#727272',
                  fill_alpha=0.5,
        ...
        # stim_side
        fig.segment(x0='horiz_line_start',y0='correct_stimside',x1='horiz_line_end',y1='correct_stimside',
                    line_color='#048e26',
                    line_alpha=0.8,
                    line_dash='dashed',
                    line_width=2,
        ...
        fig.segment(x0='horiz_line_start',y0='incorrect_stimside',x1='horiz_line_end',y1='incorrect_stimside',
                    line_color='#c91823',
                    line_alpha=0.8,
                    line_dash='dashed',
                    line_width=2,
        ...
        # reward
        fig.segment(x0='reward',y0='vert_line_start',x1='reward',y1='vert_line_end',
                    line_color='#f79413',
                    line_alpha=0.8,
                    line_width=2,
                    legend_label='Reward',
        ...
        # openloopstart
        fig.segment(x0='openloopstart',y0='vert_line_start',x1='openloopstart',y1='vert_line_end',
                    line_color='#000000',
                    line_alpha=0.8,
                    line_width=2,
        ...
        #is_opto line
        fig.segment(x0='horiz_line_start',y0=30,x1='horiz_line_end',y1=30,
                    line_color='#0a4bbb',
                    line_width=5,
                    line_alpha='is_opto',
                    source=self.cds_lines)
        ...
        fig.line(x='wheel_t',y='wheel_pos',
        ...
        fig.segment(x0='lick_t',y0=-10,x1='lick_t',y1=10,
                    line_color='#13f7f7',
                    line_width=5,
                    legend_label='Licks',
        ...
        # add hover tool
        fig.add_tools(HoverTool(names=['wheel'],
                                tooltips=[('Time','@wheel_t'),
                                          ('Position','@wheel_pos')],
                                mode='mouse'))
        self.fig = fig
    ...
    def set_cds(self,data,**kwargs):
        ...
        temp_wheel = {'wheel_t':data['wheel'][:,0],
                      'wheel_pos':data['wheel'][:,1]}

        # has the lick data
        temp_licks = {'lick_t' : data['lick'][:,0],
                     'lick_cnt' : data['lick'][:,1]}

        # has the len=2 data for drawing the zones
        temp_areas = {'stimstart': [data['stimdur'][0]]*2,
                        'stimend': [data['stimdur'][1]]*2,
                        'closedloopstart':[data['closedloopdur'][0]]*2,
                        'closedloopend':[data['closedloopdur'][1]]*2,
                        'dummy_vert_line':[-np.abs(data['stim_side'])*2,np.abs(data['stim_side'])*2]}
        ...
        if len(data['reward']):
            ...
            reward = [0]
            reward_alpha = [0]
        temp_lines = {'correct_stimside': [-1 * data['stim_side']],
...
class WheelTrace(Graph):
    ...
    def plot(self, *args,**kwargs):
        ...
        fig = bok.figure(toolbar_location=kwargs.get('toolbarlocation','above'),
                        tools = "pan,box_zoom,wheel_zoom,reset",plot_width=500, plot_height=800)
        
        for scope in self.cds_avg.keys():
            
            fig.line(x='avg_pos', y='avg_time',
                     line_color=stim_styles[scope],
                     line_width=kwargs.get('linewidth',5),
            ...
            # individual traces
            fig.multi_line(xs='indiv_pos',ys='indiv_time',
                            line_color = '#127a45',
                            line_alpha=0.3,
                ...
                for i,s in enumerate(sides,1):
                    ...
                    for sep in sep_list:
                        ...
                        if avg is not None:
                            ...
                            temp_avg = {'avg_pos' : avg[:,1],
                                        'avg_time' : avg[:,0]
                                        }

                        temp_indiv = {'indiv_pos' : [],
                                      'indiv_time' : []}
