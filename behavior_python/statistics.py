...
def mantel_haenzsel(data,stats='Q'):
    ...
    # generalized Mantel-Haenzsel test
        ...
        description = 'Generalized Mantel-Haenszel test in {0} populations with {1} response categories'.format(cr[0],cc[0])
        ...
        if I == 1 or J == 1:
            ...
            for it,key in enumerate(table_ids):
                ...
                if N[it] > 1:
                    V[key] = np.kron(N[it] * np.diag(C[key][0]) - np.matmul(np.transpose(C[key]), C[key]),
                                     N[it] * np.diag(R[key]) - np.matmul(R[key],np.transpose(R[key]))) / (N[it] * N[it] * (N[it] -1))
                else:
    ...
    # split Mantel-Haenszel test
        ...
        for split in range(2):
            ...
            if I == 1 or J == 1:
                warn_msg1[split] = 'Tables in {0} cannot be used: all but one column or one row are empty'.format(cual[split])
            else:
                if I < cr[0] or J < cc[0]:
                ...
                if num_tables == 1:
                    warn_msg2 = 'S-Q_GMH is only the sum of adjusted Pearson''s statistics when K = 2'
