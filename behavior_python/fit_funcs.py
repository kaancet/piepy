...
def naive_fit(x,y,fit_args):
    #parameter bounds
    bounds=([np.min(x),-np.inf,-np.inf,np.min(y)],
            [np.max(x),np.inf,np.inf,np.inf])
    try:
        popt, pcov = curve_fit(sigmoid,
                               x,
                               y,
                               method=fit_args.get('method','dogbox'),
                               bounds=bounds)
...
def mle_fit(data, P_model='weibull', side='right', parstart=None, parmin=None, parmax=None, nfits=5):
    """
    ...
    if parstart is None:
        ...
        if P_model == 'erf_psycho2':
            parstart = np.array([np.mean(data[0,:]), slope, .05, 1.3])
        else:
            parstart = np.array([np.mean(data[0,:]), slope, .05])
    if parmin is None:
        if P_model == 'erf_psycho2':
            parmin = np.array([np.min(data[0,:]), -10., 0., 0.])
        else:
            parmin = np.array([np.min(data[0,:]), -10., 0.])
    if parmax is None:
        if P_model == 'erf_psycho2':
            parmax = np.array([np.max(data[0,:]), 10., .4, .4])
        else:
