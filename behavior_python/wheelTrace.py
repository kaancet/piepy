...
class WheelTrace:
    ...
    def make_interval_mask(self, time_window: list = None) -> np.ndarray:
        ...
        mask = np.where(
            (time_window[0] < self.tick_t_interp)
            & (self.tick_t_interp < time_window[1])
        )
        if not len(mask[0]):
    ...
    def get_movements(
        ...
        self.vel_peak_times = np.fromiter(
            self.vel_peaks, dtype=float, count=onsets.size
        )
    ...
    def get_filtered_velocity(self, corner_frequency: float = 2, order: int = 8):
        ...
        self.velo_interp = (
            np.insert(
                np.diff(scipy.signal.sosfiltfilt(sos, self.tick_pos_interp)), 0, 0
            )
            * self.interp_freq
        )
