...
class OnePSession(Session):
    def __init__(self, sessiondir, runno: int = None):
        ...
        self.run_paths = [
            pjoin(self.data_paths.camPath, p["runname"])
            for p in self.data_paths.runPaths
        ]
    ...
    def set_run(self, run_no: int) -> None:
        try:
            ...
            raise IndexError(
                f"Tried to select the {run_no}th run, seems it doesn't exist, you have {len(self.tif_stacks)} runs in session {self.sessiondir}"
            )
    ...
    def _generate_frames_for_analysis(
        self, pre_stim_t: float, post_stim_t: float, fixed_count: int = None
    ) -> None:
        ...
        if fixed_count is None:
            ...
            if fixed_count < 0:
                raise ValueError(
                    f'Fixed frame count can"t be negative. got {fixed_count}'
                )

            if fixed_count < np.min(self.stim_mat[:, 6]):
                print(
                    f">>> WARNING <<< Fixed frame count is too low; there are trials with less then {fixed_count} frames, using {np.min(self.stim_mat[:,6])} frames instead."
                )
                min_count = np.min(self.stim_mat[:, 6])
            else:
    ...
    def get_all_run_averages(self, **kwargs):
        for run_i, run_path in enumerate(self.run_paths):
            ...
            self._generate_frames_for_analysis(
                pre_stim_t=kwargs.get("pre_stim_t", 0),
                post_stim_t=kwargs.get("post_stim_t", 0),
                fixed_count=kwargs.get("fixed_count", None),
            )

            trial_avg = self.get_run_trial_average(
            ...
            if self.pre_stim_frames:
                trial_avg[:, 0, :, :] = trial_avg[:, 0, :, :] - np.mean(
                    trial_avg[: self.pre_stim_frames + 1, 0, :, :], axis=0
                )
            else:
                trial_avg[:, 0, :, :] = trial_avg[:, 0, :, :] - np.mean(
                    trial_avg[:, 0, :, :], axis=0
            ...
            if kwargs.get("dff", True):  # do df/F
                trial_avg[:, 0, :, :] = trial_avg[:, 0, :, :] / (
                    np.mean(trial_avg[: self.pre_stim_frames + 1, 0, :, :], axis=0) + 3
                )

            # saving
            # self.trial_average = mean_mat.astype(np.float16)
            # normalize to 16bit uint
            trial_avg = cv2.normalize(
                trial_avg, None, 0, 2**16, cv2.NORM_MINMAX, cv2.CV_16U
            )

            # save average
            self.save_avg(trial_avg, run_no=run_i, **kwargs)

    def get_run_trial_average(
        self, batch_count: int = 1, downsample: int = 1, trial_count: int = None
    ) -> np.ndarray:
        ...
        mean_mat = np.zeros(
            (
                self.min_frame_count,  # min frame count per repeat/trial
                1,  # channel(gray)
                int(self.selected_stacks.shape[2] / downsample),  # height
                int(self.selected_stacks.shape[3] / downsample),
            )  # width
        )
        prev_last_frame = 0
        for curr_batch in batches:
            ...
            temp_mat = np.zeros(
                (
                    len(curr_batch),
                    1,
                    int(self.selected_stacks.shape[2] / downsample),
                    int(self.selected_stacks.shape[3] / downsample),
                )
            )
            ...
            for i in pbar:
                pbar.set_description(
                    f"Reading frames from tiff files {curr_batch[bookmark]} / {curr_batch[-1]}"
                )
                ...
                temp_mat[bookmark : bookmark + read_frames.shape[0], 0, :, :] = (
                    downed_frames
                )
    ...
    def get_reference_image(self) -> None:
        ...
        if len(ref_img):
            if len(ref_img) > 1:
                print(
                    f" >>>WARNING<<< Multiple reference images found at {self.data_paths.camPath}, using: {ref_img[0]}"
                )
            ...
            print(
                f" >>>WARNING<<< No reference image found at {self.data_paths.camPath}"
            )

    def _compare_logs(self) -> None:
        """Compares camlog and stimlog['cam']"""
        camlog_times = (
            self.selected_camlog["timestamp"].to_numpy() / 10
        )  # in 10ths of microseconds
        ...
        if camlog_frames < stimlog_cam_frames:
            print(
                f"{stimlog_cam_frames - camlog_frames} logged frame(s) not recorded by camlog!!"
            )
        elif camlog_frames > stimlog_cam_frames:
            print(
                f"{camlog_frames - stimlog_cam_frames} logged frame(s) not logged in riglog!"
            )

            self.selected_camlog = self.selected_camlog.slice(
                0, stimlog_cam_frames
            )  # removing last recorded frame
            camlog_times = (
                self.selected_camlog["timestamp"].to_numpy() / 10
            )  # in 10ths of microseconds
    ...
    def _get_screen_epochs(self) -> None:
        ...
        idx = (
            1 if screen_df[0, "value"] == 0 else 0
        )  # sometimes screen events have a 0 value entry at the beginning
        ...
        self.screen_epochs = np.hstack(
            (
                trials,
                starts.reshape(-1, 1),
                ends.reshape(-1, 1),
                trial_durs.reshape(-1, 1),
            )
        ).astype(int)
    ...
    def _correct_stimlog_times(self) -> None:
        ...
        self.selected_rawdata["vstim"] = self.selected_rawdata["vstim"].with_columns(
            corrected_times=corrected_times
        )
        self.selected_rawdata["statemachine"] = self.selected_rawdata[
            "statemachine"
        ].with_columns(corrected_times=state_corrected)
        display(f"Corrected the stimlog times by shifting {offset} seconds")

    def make_stim_matrix(
        self, extra_start_frames: int = 0, extra_end_frames: float = 0
    ) -> None:
        ...
        for i, row in enumerate(self.screen_epochs):
            ...
            stim_data = vstim.filter(
                (pl.col("corrected_times") >= epoch_times[0])
                & (pl.col("corrected_times") <= epoch_times[1])
            )

            # this is weird
            # stim_data = stim_data[stim_data['photo']!=1]
            state_data = state_machine.filter(
                (pl.col("corrected_times") >= epoch_times[0])
                & (pl.col("corrected_times") <= epoch_times[1])
            )
            ...
            assert (
                len(uniq_iStim) == 1
            ), f"More than 1 unique value in iStim {uniq_iStim}"
            assert (
                len(uniq_trial) == 1
            ), f"More than 1 unique value in iTrial {uniq_trial}"
            ...
            stimlog_cam_data = stimlog_cam.filter(
                (pl.col("duinotime") >= epoch_times[0])
            ...
            frame_times = [
                stimlog_cam_times[0] - extra_start_frames,
                stimlog_cam_times[-1] + extra_end_frames,
            ]

            if (stimlog_cam_times[0] - extra_start_frames) < prev_epoch_end:
                raise OverflowError(
                    f"The amount of  pre stim frames overflows into the previous stimulus epoch!!! {stimlog_cam_times[0]} - {extra_start_frames}"
                )
            prev_epoch_end = stimlog_cam_times[-1]

            # weird way but ok
            self.stim_mat[i, :] = (
                [i + 1] + epoch_props + frame_times + [np.diff(frame_times)[0] + 1]
            )
    ...
    def register_run(self, overwrite: bool = False) -> None:
        ...
        if os.path.exists(reg_path):
            if len(os.listdir(reg_path)) + 1 == len(
                os.listdir(self.run_path)
            ):  # +1 is to account for the camlog
        ...
        for i in pbar:
            ...
            stack_idx = [
                self.selected_stacks.frames_offset[i],
                self.selected_stacks.frames_offset[i + 1],
            ]
            ...
            for j in range(stack.shape[0]):
                ...
                pbar.set_description(
                    f"Registering {stack_name}, frames {j}/{stack.shape[0]}"
                )
