...
class GSheet:
    def __init__(self,spreadsheet_name,scope=scope,cred_path=cred_path):
        ...
        self.creds = ServiceAccountCredentials.from_json_keyfile_name(self.cred_path, self.scope)
    ...
    def write_cell(self,cell,value):
        ...
        try:
            result = self.service.spreadsheets().values().update(
                spreadsheetId=self.sheet.id, range=cell,
    ...
    def read_cell(self,cell):
        ...
        result = self.service.spreadsheets().values().get(
            spreadsheetId=self.sheet.id,range=cell).execute()
